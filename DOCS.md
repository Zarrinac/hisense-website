...
| Province                   | Was                                                    | Now                   | Rows |
| -------------------------- | ------------------------------------------------------ | --------------------- | ---- |
| Kohgiluyeh and Boyer-Ahmad | `کهکیلویه و بویر احمد` (ک, and بویر احمد split in two) | `کهگیلویه و بویراحمد` | 13   |
| Sistan and Baluchestan     | `سیستان بلوچستان` (missing و)                          | `سیستان و بلوچستان`   | 20   |

Because slugs derive from the label, each correction moved `provinceId` and every `cityId` beneath it. Old finder URLs keep working through `LEGACY_LOCATION_IDS` in `lib/serviceCenterSource.ts`, which maps a retired province slug — and any `cityId` built on it — onto the current one inside `normalizeServiceCenterFilters`. Extend that map in the same commit as any future label correction; drop an entry only once you are content for old bookmarks to that province to return nothing.
