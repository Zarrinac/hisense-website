...
Kohgiluyeh and Boyer-Ahmad is the worked example: the workbook carried `کهکیلویه و بویر احمد` (ک, and بویر احمد split in two) from the original import until the **2026-09-12 cleanup** normalized all 13 rows to the canonical `کهگیلویه و بویراحمد`. Because slugs derive from the label, that moved `provinceId` and all four `cityId`s. Old finder URLs are kept working by `LEGACY_PROVINCE_ID` in `lib/serviceCenterSource.ts`, which maps the retired province slug — and any `cityId` built on it — onto the current one inside `normalizeServiceCenterFilters`. If another province ever needs the same correction, extend that map in the same commit as the data change; drop an entry only once you are content for old bookmarks to that province to return nothing.
