import type { Locale } from '@/i18n/routing';

// Generated from the iran-city dataset (v1.2.2) to keep province/city coverage current
// while preserving the app's existing helper API and search behavior.

type LocalizedLabel = {
  en: string;
  fa: string;
};

export type IranCity = {
  id: string;
  labels: LocalizedLabel;
};

export type IranProvince = {
  id: string;
  labels: LocalizedLabel;
  cities: IranCity[];
};

export const iranProvinces: IranProvince[] = [
  {
    id: 'east-azerbaijan',
    labels: {
      fa: 'آذربایجان شرقی',
      en: 'East Azerbaijan',
    },
    cities: [
      {
        id: '1',
        labels: {
          fa: 'اسکو',
          en: 'Osku',
        },
      },
      {
        id: '2',
        labels: {
          fa: 'اهر',
          en: 'Ahar',
        },
      },
      {
        id: '3',
        labels: {
          fa: 'ایلخچی',
          en: 'Ilkhchi',
        },
      },
      {
        id: '4',
        labels: {
          fa: 'آبش احمد',
          en: 'Abesh Ahmad',
        },
      },
      {
        id: '5',
        labels: {
          fa: 'آذرشهر',
          en: 'Azarshahr',
        },
      },
      {
        id: '6',
        labels: {
          fa: 'آقکند',
          en: 'Aq Kand',
        },
      },
      {
        id: '7',
        labels: {
          fa: 'باسمنج',
          en: 'Basmenj',
        },
      },
      {
        id: '8',
        labels: {
          fa: 'بخشایش',
          en: 'Bakhshayesh',
        },
      },
      {
        id: '9',
        labels: {
          fa: 'بستان آباد',
          en: 'Bostanabad',
        },
      },
      {
        id: '10',
        labels: {
          fa: 'بناب',
          en: 'Bonab',
        },
      },
      {
        id: '11',
        labels: {
          fa: 'بناب جدید',
          en: 'Bonab Jadid',
        },
      },
      {
        id: '12',
        labels: {
          fa: 'تبریز',
          en: 'Tabriz',
        },
      },
      {
        id: '13',
        labels: {
          fa: 'ترک',
          en: 'Tark',
        },
      },
      {
        id: '14',
        labels: {
          fa: 'ترکمانچای',
          en: 'Torkamanchay',
        },
      },
      {
        id: '15',
        labels: {
          fa: 'تسوج',
          en: 'Tasuj',
        },
      },
      {
        id: '16',
        labels: {
          fa: 'تیکمه داش',
          en: 'Tekmeh Dash',
        },
      },
      {
        id: '17',
        labels: {
          fa: 'جلفا',
          en: 'Jolfa',
        },
      },
      {
        id: '18',
        labels: {
          fa: 'خاروانا',
          en: 'Kharvana',
        },
      },
      {
        id: '19',
        labels: {
          fa: 'خامنه',
          en: 'Khameneh',
        },
      },
      {
        id: '20',
        labels: {
          fa: 'خراجو',
          en: 'Kharaju',
        },
      },
      {
        id: '21',
        labels: {
          fa: 'خسروشهر',
          en: 'Khosroshahr',
        },
      },
      {
        id: '22',
        labels: {
          fa: 'خضرلو',
          en: 'Khezerlu',
        },
      },
      {
        id: '23',
        labels: {
          fa: 'خمارلو',
          en: 'Khomarlu',
        },
      },
      {
        id: '24',
        labels: {
          fa: 'خواجه',
          en: 'Khvajeh',
        },
      },
      {
        id: '25',
        labels: {
          fa: 'دوزدوزان',
          en: 'Duzduzan',
        },
      },
      {
        id: '26',
        labels: {
          fa: 'زرنق',
          en: 'Zarnaq',
        },
      },
      {
        id: '27',
        labels: {
          fa: 'زنوز',
          en: 'Zonuz',
        },
      },
      {
        id: '28',
        labels: {
          fa: 'سراب',
          en: 'Sarab',
        },
      },
      {
        id: '29',
        labels: {
          fa: 'سردرود',
          en: 'Sardrud',
        },
      },
      {
        id: '30',
        labels: {
          fa: 'سهند',
          en: 'Sahand',
        },
      },
      {
        id: '31',
        labels: {
          fa: 'سیس',
          en: 'Sis',
        },
      },
      {
        id: '32',
        labels: {
          fa: 'سیه رود',
          en: 'Siah Rud',
        },
      },
      {
        id: '33',
        labels: {
          fa: 'شبستر',
          en: 'Shabestar',
        },
      },
      {
        id: '34',
        labels: {
          fa: 'شربیان',
          en: 'Sharabian',
        },
      },
      {
        id: '35',
        labels: {
          fa: 'شرفخانه',
          en: 'Bandar-e Sharafkhaneh',
        },
      },
      {
        id: '36',
        labels: {
          fa: 'شندآباد',
          en: 'Shendabad',
        },
      },
      {
        id: '37',
        labels: {
          fa: 'صوفیان',
          en: 'Sufian',
        },
      },
      {
        id: '38',
        labels: {
          fa: 'عجب شیر',
          en: 'Ajabshir',
        },
      },
      {
        id: '39',
        labels: {
          fa: 'قره آغاج',
          en: 'Qarah Aghaj',
        },
      },
      {
        id: '40',
        labels: {
          fa: 'کشکسرای',
          en: 'Koshksaray',
        },
      },
      {
        id: '41',
        labels: {
          fa: 'کلوانق',
          en: 'Kolvanaq',
        },
      },
      {
        id: '42',
        labels: {
          fa: 'کلیبر',
          en: 'Kaleybar',
        },
      },
      {
        id: '43',
        labels: {
          fa: 'کوزه کنان',
          en: 'Kuzeh Kanan',
        },
      },
      {
        id: '44',
        labels: {
          fa: 'گوگان',
          en: 'Gugan',
        },
      },
      {
        id: '45',
        labels: {
          fa: 'لیلان',
          en: 'Leylan',
        },
      },
      {
        id: '46',
        labels: {
          fa: 'مراغه',
          en: 'Maragheh',
        },
      },
      {
        id: '47',
        labels: {
          fa: 'مرند',
          en: 'Marand',
        },
      },
      {
        id: '48',
        labels: {
          fa: 'ملکان',
          en: 'Malekan',
        },
      },
      {
        id: '49',
        labels: {
          fa: 'ملک کیان',
          en: 'Malek Kian',
        },
      },
      {
        id: '50',
        labels: {
          fa: 'ممقان',
          en: 'Mamqan',
        },
      },
      {
        id: '51',
        labels: {
          fa: 'مهربان',
          en: 'Mehraban',
        },
      },
      {
        id: '52',
        labels: {
          fa: 'میانه',
          en: 'Meyaneh',
        },
      },
      {
        id: '53',
        labels: {
          fa: 'نظرکهریزی',
          en: 'Nazar Kahriz',
        },
      },
      {
        id: '54',
        labels: {
          fa: 'هادی شهر',
          en: 'Hadishahr',
        },
      },
      {
        id: '55',
        labels: {
          fa: 'هرگلان',
          en: 'Hargalan',
        },
      },
      {
        id: '56',
        labels: {
          fa: 'هریس',
          en: 'Heris',
        },
      },
      {
        id: '57',
        labels: {
          fa: 'هشترود',
          en: 'Hashtrud',
        },
      },
      {
        id: '58',
        labels: {
          fa: 'هوراند',
          en: 'Hurand',
        },
      },
      {
        id: '59',
        labels: {
          fa: 'وایقان',
          en: 'Vayqan',
        },
      },
      {
        id: '60',
        labels: {
          fa: 'ورزقان',
          en: 'Varzaqan',
        },
      },
      {
        id: '61',
        labels: {
          fa: 'یامچی',
          en: 'Yamchi',
        },
      },
    ],
  },
  {
    id: 'west-azerbaijan',
    labels: {
      fa: 'آذربایجان غربی',
      en: 'West Azerbaijan',
    },
    cities: [
      {
        id: '62',
        labels: {
          fa: 'ارومیه',
          en: 'Urmia',
        },
      },
      {
        id: '63',
        labels: {
          fa: 'اشنویه',
          en: 'Oshnavieh',
        },
      },
      {
        id: '64',
        labels: {
          fa: 'ایواوغلی',
          en: 'Ivaowghli',
        },
      },
      {
        id: '65',
        labels: {
          fa: 'آواجیق',
          en: 'Avajiq',
        },
      },
      {
        id: '66',
        labels: {
          fa: 'باروق',
          en: 'Baruq',
        },
      },
      {
        id: '67',
        labels: {
          fa: 'بازرگان',
          en: 'Bazargan',
        },
      },
      {
        id: '68',
        labels: {
          fa: 'بوکان',
          en: 'Bukan',
        },
      },
      {
        id: '69',
        labels: {
          fa: 'پلدشت',
          en: 'Poldasht',
        },
      },
      {
        id: '70',
        labels: {
          fa: 'پیرانشهر',
          en: 'Piranshahr',
        },
      },
      {
        id: '71',
        labels: {
          fa: 'تازه شهر',
          en: 'Tazeh Shahr',
        },
      },
      {
        id: '72',
        labels: {
          fa: 'تکاب',
          en: 'Takab',
        },
      },
      {
        id: '73',
        labels: {
          fa: 'چهاربرج',
          en: 'Chahar Borj',
        },
      },
      {
        id: '74',
        labels: {
          fa: 'خوی',
          en: 'Khoy',
        },
      },
      {
        id: '75',
        labels: {
          fa: 'دیزج دیز',
          en: 'Diz Jadiz',
        },
      },
      {
        id: '76',
        labels: {
          fa: 'ربط',
          en: 'Rabat',
        },
      },
      {
        id: '77',
        labels: {
          fa: 'سردشت',
          en: 'Sardasht',
        },
      },
      {
        id: '78',
        labels: {
          fa: 'سرو',
          en: 'Serow',
        },
      },
      {
        id: '79',
        labels: {
          fa: 'سلماس',
          en: 'Salmas',
        },
      },
      {
        id: '80',
        labels: {
          fa: 'سیلوانه',
          en: 'Silvaneh',
        },
      },
      {
        id: '81',
        labels: {
          fa: 'سیمینه',
          en: 'Simmineh',
        },
      },
      {
        id: '82',
        labels: {
          fa: 'سیه چشمه',
          en: 'Seyah Cheshmeh',
        },
      },
      {
        id: '83',
        labels: {
          fa: 'شاهین دژ',
          en: 'Shahin Dezh',
        },
      },
      {
        id: '84',
        labels: {
          fa: 'شوط',
          en: 'Showt',
        },
      },
      {
        id: '85',
        labels: {
          fa: 'فیرورق',
          en: 'Firuraq',
        },
      },
      {
        id: '86',
        labels: {
          fa: 'قره ضیاءالدین',
          en: "Qarah Zia' od Din",
        },
      },
      {
        id: '87',
        labels: {
          fa: 'قطور',
          en: 'Qatur',
        },
      },
      {
        id: '88',
        labels: {
          fa: 'قوشچی',
          en: 'Qushchi',
        },
      },
      {
        id: '89',
        labels: {
          fa: 'کشاورز',
          en: 'Keshavarz',
        },
      },
      {
        id: '90',
        labels: {
          fa: 'گردکشانه',
          en: 'Gerd Ka Shaneh',
        },
      },
      {
        id: '91',
        labels: {
          fa: 'ماکو',
          en: 'Maku',
        },
      },
      {
        id: '92',
        labels: {
          fa: 'محمدیار',
          en: 'Mohammad Yar',
        },
      },
      {
        id: '93',
        labels: {
          fa: 'محمودآباد',
          en: 'Mahmudabad',
        },
      },
      {
        id: '94',
        labels: {
          fa: 'مهاباد',
          en: 'Mahabad',
        },
      },
      {
        id: '95',
        labels: {
          fa: 'میاندوآب',
          en: 'Miandoab',
        },
      },
      {
        id: '96',
        labels: {
          fa: 'میرآباد',
          en: 'Mirabad',
        },
      },
      {
        id: '97',
        labels: {
          fa: 'نالوس',
          en: 'Nalus',
        },
      },
      {
        id: '98',
        labels: {
          fa: 'نقده',
          en: 'Naqadeh',
        },
      },
      {
        id: '99',
        labels: {
          fa: 'نوشین',
          en: 'Nushin Shar',
        },
      },
    ],
  },
  {
    id: 'ardabil',
    labels: {
      fa: 'اردبیل',
      en: 'Ardabil',
    },
    cities: [
      {
        id: '100',
        labels: {
          fa: 'اردبیل',
          en: 'Ardabil',
        },
      },
      {
        id: '101',
        labels: {
          fa: 'اصلاندوز',
          en: 'Aslan Duz',
        },
      },
      {
        id: '102',
        labels: {
          fa: 'آبی بیگلو',
          en: 'Abi Beyglu',
        },
      },
      {
        id: '103',
        labels: {
          fa: 'بیله سوار',
          en: 'Bileh Savar',
        },
      },
      {
        id: '104',
        labels: {
          fa: 'پارس آباد',
          en: 'Parsabad',
        },
      },
      {
        id: '105',
        labels: {
          fa: 'تازه کند',
          en: 'Tazeh Kand',
        },
      },
      {
        id: '106',
        labels: {
          fa: 'تازه کندانگوت',
          en: 'Tazeh Kand-e Angut',
        },
      },
      {
        id: '107',
        labels: {
          fa: 'جعفرآباد',
          en: "Ja'farabad",
        },
      },
      {
        id: '108',
        labels: {
          fa: 'خلخال',
          en: 'Khalkhal',
        },
      },
      {
        id: '109',
        labels: {
          fa: 'رضی',
          en: 'Razi',
        },
      },
      {
        id: '110',
        labels: {
          fa: 'سرعین',
          en: 'Sareyn',
        },
      },
      {
        id: '111',
        labels: {
          fa: 'عنبران',
          en: 'Anbaran',
        },
      },
      {
        id: '112',
        labels: {
          fa: 'فخرآباد',
          en: 'Fakhrabad',
        },
      },
      {
        id: '113',
        labels: {
          fa: 'کلور',
          en: 'Kolur',
        },
      },
      {
        id: '114',
        labels: {
          fa: 'کوراییم',
          en: 'Kuraim',
        },
      },
      {
        id: '115',
        labels: {
          fa: 'گرمی',
          en: 'Germi',
        },
      },
      {
        id: '116',
        labels: {
          fa: 'گیوی',
          en: 'Givi',
        },
      },
      {
        id: '117',
        labels: {
          fa: 'لاهرود',
          en: 'Lahrud',
        },
      },
      {
        id: '118',
        labels: {
          fa: 'مشگین شهر',
          en: 'Meshgin Shahr',
        },
      },
      {
        id: '119',
        labels: {
          fa: 'نمین',
          en: 'Namin',
        },
      },
      {
        id: '120',
        labels: {
          fa: 'نیر',
          en: 'Nir',
        },
      },
      {
        id: '121',
        labels: {
          fa: 'هشتجین',
          en: 'Hashatjin',
        },
      },
      {
        id: '122',
        labels: {
          fa: 'هیر',
          en: 'Hir',
        },
      },
    ],
  },
  {
    id: 'isfahan',
    labels: {
      fa: 'اصفهان',
      en: 'Isfahan',
    },
    cities: [
      {
        id: '123',
        labels: {
          fa: 'ابریشم',
          en: 'Abrisham',
        },
      },
      {
        id: '124',
        labels: {
          fa: 'ابوزیدآباد',
          en: 'Abuzeydabad',
        },
      },
      {
        id: '125',
        labels: {
          fa: 'اردستان',
          en: 'Ardestan',
        },
      },
      {
        id: '126',
        labels: {
          fa: 'اژیه',
          en: 'Ezhiyeh',
        },
      },
      {
        id: '127',
        labels: {
          fa: 'اصفهان',
          en: 'Isfahan',
        },
      },
      {
        id: '128',
        labels: {
          fa: 'افوس',
          en: 'Afus',
        },
      },
      {
        id: '129',
        labels: {
          fa: 'انارک',
          en: 'Anarak',
        },
      },
      {
        id: '130',
        labels: {
          fa: 'ایمانشهر',
          en: 'Imanshahr',
        },
      },
      {
        id: '131',
        labels: {
          fa: 'آران وبیدگل',
          en: 'Aran O Bidgol',
        },
      },
      {
        id: '132',
        labels: {
          fa: 'بادرود',
          en: 'Bad Rud',
        },
      },
      {
        id: '133',
        labels: {
          fa: 'باغ بهادران',
          en: 'Bagh-e Bahadaran',
        },
      },
      {
        id: '134',
        labels: {
          fa: 'بافران',
          en: 'Bafran',
        },
      },
      {
        id: '135',
        labels: {
          fa: 'برزک',
          en: 'Barzok',
        },
      },
      {
        id: '136',
        labels: {
          fa: 'برف انبار',
          en: 'Barf Anbar',
        },
      },
      {
        id: '137',
        labels: {
          fa: 'بهاران شهر',
          en: 'Baharan Shahr',
        },
      },
      {
        id: '138',
        labels: {
          fa: 'بهارستان',
          en: 'Baharestan',
        },
      },
      {
        id: '139',
        labels: {
          fa: 'بوئین و میاندشت',
          en: 'Buin Va Miandasht',
        },
      },
      {
        id: '140',
        labels: {
          fa: 'پیربکران',
          en: 'Pir Bakran',
        },
      },
      {
        id: '141',
        labels: {
          fa: 'تودشک',
          en: 'Tudeshk',
        },
      },
      {
        id: '142',
        labels: {
          fa: 'تیران',
          en: 'Tiran',
        },
      },
      {
        id: '143',
        labels: {
          fa: 'جندق',
          en: 'Jandaq',
        },
      },
      {
        id: '144',
        labels: {
          fa: 'جوزدان',
          en: 'Jowzdan',
        },
      },
      {
        id: '145',
        labels: {
          fa: 'جوشقان و کامو',
          en: 'Jowsheqan va Kamu',
        },
      },
      {
        id: '146',
        labels: {
          fa: 'چادگان',
          en: 'Chadegan',
        },
      },
      {
        id: '147',
        labels: {
          fa: 'چرمهین',
          en: 'Charmahin',
        },
      },
      {
        id: '148',
        labels: {
          fa: 'چمگردان',
          en: 'Chamgordan',
        },
      },
      {
        id: '149',
        labels: {
          fa: 'حبیب آباد',
          en: 'Habibabad',
        },
      },
      {
        id: '150',
        labels: {
          fa: 'حسن آباد',
          en: 'Hasanabad',
        },
      },
      {
        id: '151',
        labels: {
          fa: 'حنا',
          en: 'Hana',
        },
      },
      {
        id: '152',
        labels: {
          fa: 'خالدآباد',
          en: 'Khaledabad',
        },
      },
      {
        id: '153',
        labels: {
          fa: 'خمینی شهر',
          en: 'Khomeyni Shahr',
        },
      },
      {
        id: '154',
        labels: {
          fa: 'خوانسار',
          en: 'Khvansar',
        },
      },
      {
        id: '155',
        labels: {
          fa: 'خور',
          en: 'Khur',
        },
      },
      {
        id: '157',
        labels: {
          fa: 'خورزوق',
          en: 'Khowrzuq',
        },
      },
      {
        id: '158',
        labels: {
          fa: 'داران',
          en: 'Daran',
        },
      },
      {
        id: '159',
        labels: {
          fa: 'دامنه',
          en: 'Damaneh',
        },
      },
      {
        id: '160',
        labels: {
          fa: 'درچه',
          en: 'Dorcheh',
        },
      },
      {
        id: '161',
        labels: {
          fa: 'دستگرد',
          en: 'Dastgerd',
        },
      },
      {
        id: '162',
        labels: {
          fa: 'دهاقان',
          en: 'Dehaqan',
        },
      },
      {
        id: '163',
        labels: {
          fa: 'دهق',
          en: 'Dehaq',
        },
      },
      {
        id: '164',
        labels: {
          fa: 'دولت آباد',
          en: 'Dowlatabad',
        },
      },
      {
        id: '165',
        labels: {
          fa: 'دیزیچه',
          en: 'Dizicheh',
        },
      },
      {
        id: '166',
        labels: {
          fa: 'رزوه',
          en: 'Rezveh',
        },
      },
      {
        id: '167',
        labels: {
          fa: 'رضوانشهر',
          en: 'Rezvanshahr',
        },
      },
      {
        id: '168',
        labels: {
          fa: 'زاینده رود',
          en: 'Zayandeh Rud',
        },
      },
      {
        id: '169',
        labels: {
          fa: 'زرین شهر',
          en: 'Zarin Shahr',
        },
      },
      {
        id: '170',
        labels: {
          fa: 'زواره',
          en: 'Zavareh',
        },
      },
      {
        id: '171',
        labels: {
          fa: 'زیباشهر',
          en: 'Zibashahr',
        },
      },
      {
        id: '172',
        labels: {
          fa: 'سده لنجان',
          en: 'Sedeh Lanjan',
        },
      },
      {
        id: '173',
        labels: {
          fa: 'سفیدشهر',
          en: 'Sefidshahr',
        },
      },
      {
        id: '174',
        labels: {
          fa: 'سگزی',
          en: 'Sagzi',
        },
      },
      {
        id: '175',
        labels: {
          fa: 'سمیرم',
          en: 'Semirom',
        },
      },
      {
        id: '176',
        labels: {
          fa: 'شاهین شهر',
          en: 'Shahin Shahr',
        },
      },
      {
        id: '177',
        labels: {
          fa: 'شهرضا',
          en: 'Shahreza',
        },
      },
      {
        id: '178',
        labels: {
          fa: 'طالخونچه',
          en: 'Talkhuncheh',
        },
      },
      {
        id: '179',
        labels: {
          fa: 'عسگران',
          en: 'Asgaran',
        },
      },
      {
        id: '180',
        labels: {
          fa: 'علویجه',
          en: 'Alavijeh',
        },
      },
      {
        id: '181',
        labels: {
          fa: 'فرخی',
          en: 'Farrokhi',
        },
      },
      {
        id: '182',
        labels: {
          fa: 'فریدونشهر',
          en: 'Fereydunshahr',
        },
      },
      {
        id: '183',
        labels: {
          fa: 'فلاورجان',
          en: 'Falavarjan',
        },
      },
      {
        id: '184',
        labels: {
          fa: 'فولادشهر',
          en: 'Fulad Shahr',
        },
      },
      {
        id: '185',
        labels: {
          fa: 'قمصر',
          en: 'Qamsar',
        },
      },
      {
        id: '186',
        labels: {
          fa: 'قهجاورستان',
          en: 'Qahjavarestan',
        },
      },
      {
        id: '187',
        labels: {
          fa: 'قهدریجان',
          en: 'Qahderijan',
        },
      },
      {
        id: '188',
        labels: {
          fa: 'کاشان',
          en: 'Kashan',
        },
      },
      {
        id: '189',
        labels: {
          fa: 'کرکوند',
          en: 'Karkevand',
        },
      },
      {
        id: '190',
        labels: {
          fa: 'کلیشاد و سودرجان',
          en: 'Kelishad va Sudarjan',
        },
      },
      {
        id: '191',
        labels: {
          fa: 'کمشچه',
          en: 'Komeshcheh',
        },
      },
      {
        id: '192',
        labels: {
          fa: 'کمه',
          en: 'Kommeh',
        },
      },
      {
        id: '193',
        labels: {
          fa: 'کهریزسنگ',
          en: 'Kahriz Sang',
        },
      },
      {
        id: '194',
        labels: {
          fa: 'کوشک',
          en: 'Kushk',
        },
      },
      {
        id: '195',
        labels: {
          fa: 'کوهپایه',
          en: 'Kuhpayeh',
        },
      },
      {
        id: '196',
        labels: {
          fa: 'گرگاب',
          en: 'Gorgab',
        },
      },
      {
        id: '197',
        labels: {
          fa: 'گزبرخوار',
          en: 'Gaz-e Borkhar',
        },
      },
      {
        id: '198',
        labels: {
          fa: 'گلپایگان',
          en: 'Golpayegan',
        },
      },
      {
        id: '199',
        labels: {
          fa: 'گلدشت',
          en: 'Goldasht',
        },
      },
      {
        id: '200',
        labels: {
          fa: 'گلشهر',
          en: 'Golshahr',
        },
      },
      {
        id: '201',
        labels: {
          fa: 'گوگد',
          en: 'Guged',
        },
      },
      {
        id: '202',
        labels: {
          fa: 'لای بید',
          en: 'Lay Bid',
        },
      },
      {
        id: '203',
        labels: {
          fa: 'مبارکه',
          en: 'Mobarakeh',
        },
      },
      {
        id: '204',
        labels: {
          fa: 'مجلسی',
          en: 'Majlesi',
        },
      },
      {
        id: '205',
        labels: {
          fa: 'محمدآباد',
          en: 'Mohammadabad',
        },
      },
      {
        id: '206',
        labels: {
          fa: 'مشکات',
          en: 'Meshkat',
        },
      },
      {
        id: '207',
        labels: {
          fa: 'منظریه',
          en: 'Manzariyeh',
        },
      },
      {
        id: '208',
        labels: {
          fa: 'مهاباد',
          en: 'Mahabad',
        },
      },
      {
        id: '209',
        labels: {
          fa: 'میمه',
          en: 'Meymeh',
        },
      },
      {
        id: '210',
        labels: {
          fa: 'نائین',
          en: "Na'in",
        },
      },
      {
        id: '211',
        labels: {
          fa: 'نجف آباد',
          en: 'Najafabad',
        },
      },
      {
        id: '212',
        labels: {
          fa: 'نصرآباد',
          en: 'Nasrabad-e Jiruyeh',
        },
      },
      {
        id: '213',
        labels: {
          fa: 'نطنز',
          en: 'Natanz',
        },
      },
      {
        id: '214',
        labels: {
          fa: 'نوش آباد',
          en: 'Nushabad',
        },
      },
      {
        id: '215',
        labels: {
          fa: 'نیاسر',
          en: 'Neyasar',
        },
      },
      {
        id: '216',
        labels: {
          fa: 'نیک آباد',
          en: 'Nikabad',
        },
      },
      {
        id: '217',
        labels: {
          fa: 'هرند',
          en: 'Harand',
        },
      },
      {
        id: '218',
        labels: {
          fa: 'ورزنه',
          en: 'Varzaneh',
        },
      },
      {
        id: '219',
        labels: {
          fa: 'ورنامخواست',
          en: 'Varnamkhvast',
        },
      },
      {
        id: '220',
        labels: {
          fa: 'وزوان',
          en: 'Vazvan',
        },
      },
      {
        id: '221',
        labels: {
          fa: 'ونک',
          en: 'Vanak',
        },
      },
    ],
  },
  {
    id: 'alborz',
    labels: {
      fa: 'البرز',
      en: 'Alborz',
    },
    cities: [
      {
        id: '222',
        labels: {
          fa: 'اسارا',
          en: 'Asara',
        },
      },
      {
        id: '223',
        labels: {
          fa: 'اشتهارد',
          en: 'Eshtehard',
        },
      },
      {
        id: '224',
        labels: {
          fa: 'تنکمان',
          en: 'Tankaman',
        },
      },
      {
        id: '225',
        labels: {
          fa: 'چهارباغ',
          en: 'Chahar Dangeh',
        },
      },
      {
        id: '226',
        labels: {
          fa: 'سعید آباد',
          en: "Sa'idabad",
        },
      },
      {
        id: '227',
        labels: {
          fa: 'شهر جدید هشتگرد',
          en: 'Shahr-e Jadid-e Hastgerd',
        },
      },
      {
        id: '228',
        labels: {
          fa: 'طالقان',
          en: 'Taleqan',
        },
      },
      {
        id: '229',
        labels: {
          fa: 'کرج',
          en: 'Karaj',
        },
      },
      {
        id: '230',
        labels: {
          fa: 'کمال شهر',
          en: 'Kamalshahr',
        },
      },
      {
        id: '231',
        labels: {
          fa: 'کوهسار',
          en: 'Kuhsar',
        },
      },
      {
        id: '232',
        labels: {
          fa: 'گرمدره',
          en: 'Garmdarreh',
        },
      },
      {
        id: '233',
        labels: {
          fa: 'ماهدشت',
          en: 'Mahdasht',
        },
      },
      {
        id: '234',
        labels: {
          fa: 'محمدشهر',
          en: 'Mohammad Shahr',
        },
      },
      {
        id: '235',
        labels: {
          fa: 'مشکین دشت',
          en: 'Meshkin Dasht',
        },
      },
      {
        id: '236',
        labels: {
          fa: 'نظرآباد',
          en: 'Nazaraabad',
        },
      },
      {
        id: '237',
        labels: {
          fa: 'هشتگرد',
          en: 'Hashtgerd',
        },
      },
      {
        id: '1117',
        labels: {
          fa: 'فردیس',
          en: 'Fardis',
        },
      },
      {
        id: '1118',
        labels: {
          fa: 'مارلیک',
          en: 'Marlik',
        },
      },
      {
        id: '1135',
        labels: {
          fa: 'کردان',
          en: 'Kordan',
        },
      },
      {
        id: '1137',
        labels: {
          fa: 'ساوجبلاغ',
          en: 'Savojbolagh',
        },
      },
      {
        id: '1138',
        labels: {
          fa: 'تهران دشت',
          en: 'Tehrandasht',
        },
      },
    ],
  },
  {
    id: 'ilam',
    labels: {
      fa: 'ایلام',
      en: 'Ilam',
    },
    cities: [
      {
        id: '238',
        labels: {
          fa: 'ارکواز',
          en: 'Arkavaz',
        },
      },
      {
        id: '239',
        labels: {
          fa: 'ایلام',
          en: 'Ilam',
        },
      },
      {
        id: '240',
        labels: {
          fa: 'ایوان',
          en: 'Eyvan',
        },
      },
      {
        id: '241',
        labels: {
          fa: 'آبدانان',
          en: 'Abdanan',
        },
      },
      {
        id: '242',
        labels: {
          fa: 'آسمان آباد',
          en: 'Asmanabad',
        },
      },
      {
        id: '243',
        labels: {
          fa: 'بدره',
          en: 'Badreh',
        },
      },
      {
        id: '244',
        labels: {
          fa: 'پهله',
          en: 'Pahleh',
        },
      },
      {
        id: '245',
        labels: {
          fa: 'توحید',
          en: 'Towhid',
        },
      },
      {
        id: '246',
        labels: {
          fa: 'چوار',
          en: 'Chavar',
        },
      },
      {
        id: '247',
        labels: {
          fa: 'دره شهر',
          en: 'Darreh Shahr',
        },
      },
      {
        id: '248',
        labels: {
          fa: 'دلگشا',
          en: 'Delgosha',
        },
      },
      {
        id: '249',
        labels: {
          fa: 'دهلران',
          en: 'Dehloran',
        },
      },
      {
        id: '250',
        labels: {
          fa: 'زرنه',
          en: 'Zarneh',
        },
      },
      {
        id: '251',
        labels: {
          fa: 'سراب باغ',
          en: 'Sarab Bagh',
        },
      },
      {
        id: '252',
        labels: {
          fa: 'سرابله',
          en: 'Sarableh',
        },
      },
      {
        id: '253',
        labels: {
          fa: 'صالح آباد',
          en: 'Salehabad',
        },
      },
      {
        id: '254',
        labels: {
          fa: 'لومار',
          en: 'Lumar',
        },
      },
      {
        id: '255',
        labels: {
          fa: 'مهران',
          en: 'Mehran',
        },
      },
      {
        id: '256',
        labels: {
          fa: 'مورموری',
          en: 'Murmuri',
        },
      },
      {
        id: '257',
        labels: {
          fa: 'موسیان',
          en: 'Musian',
        },
      },
      {
        id: '258',
        labels: {
          fa: 'میمه',
          en: 'Meymeh',
        },
      },
    ],
  },
  {
    id: 'bushehr',
    labels: {
      fa: 'بوشهر',
      en: 'Bushehr',
    },
    cities: [
      {
        id: '259',
        labels: {
          fa: 'امام حسن',
          en: 'Emam Hasan',
        },
      },
      {
        id: '260',
        labels: {
          fa: 'انارستان',
          en: 'Anarestan',
        },
      },
      {
        id: '261',
        labels: {
          fa: 'اهرم',
          en: 'Ahram',
        },
      },
      {
        id: '262',
        labels: {
          fa: 'آب پخش',
          en: 'Ab Pakhsh',
        },
      },
      {
        id: '263',
        labels: {
          fa: 'آبدان',
          en: 'Abdan',
        },
      },
      {
        id: '264',
        labels: {
          fa: 'برازجان',
          en: 'Borazjan',
        },
      },
      {
        id: '265',
        labels: {
          fa: 'بردخون',
          en: 'Bord Khun',
        },
      },
      {
        id: '266',
        labels: {
          fa: 'بندردیر',
          en: 'Deyr',
        },
      },
      {
        id: '267',
        labels: {
          fa: 'بندردیلم',
          en: 'Bandar-e Deylam',
        },
      },
      {
        id: '268',
        labels: {
          fa: 'بندرریگ',
          en: 'Bandar-e Rig',
        },
      },
      {
        id: '269',
        labels: {
          fa: 'بندرکنگان',
          en: 'Bandar Kangan',
        },
      },
      {
        id: '270',
        labels: {
          fa: 'بندرگناوه',
          en: 'Bandar-e Genaveh',
        },
      },
      {
        id: '271',
        labels: {
          fa: 'بنک',
          en: 'Banak',
        },
      },
      {
        id: '272',
        labels: {
          fa: 'بوشهر',
          en: 'Bushehr',
        },
      },
      {
        id: '273',
        labels: {
          fa: 'تنگ ارم',
          en: 'Tang-e Eram',
        },
      },
      {
        id: '274',
        labels: {
          fa: 'جم',
          en: 'Jam',
        },
      },
      {
        id: '275',
        labels: {
          fa: 'چغادک',
          en: 'Choghadak',
        },
      },
      {
        id: '276',
        labels: {
          fa: 'خارک',
          en: 'Khark',
        },
      },
      {
        id: '277',
        labels: {
          fa: 'خورموج',
          en: 'Khormoj',
        },
      },
      {
        id: '278',
        labels: {
          fa: 'دالکی',
          en: 'Dalaki',
        },
      },
      {
        id: '279',
        labels: {
          fa: 'دلوار',
          en: 'Delvar',
        },
      },
      {
        id: '280',
        labels: {
          fa: 'ریز',
          en: 'Riz',
        },
      },
      {
        id: '281',
        labels: {
          fa: 'سعدآباد',
          en: "Sa'dabad",
        },
      },
      {
        id: '282',
        labels: {
          fa: 'سیراف',
          en: 'Bandar Siraf',
        },
      },
      {
        id: '283',
        labels: {
          fa: 'شبانکاره',
          en: 'Shabankareh',
        },
      },
      {
        id: '284',
        labels: {
          fa: 'شنبه',
          en: 'Shanbeh',
        },
      },
      {
        id: '285',
        labels: {
          fa: 'عسلویه',
          en: 'Asaluyeh',
        },
      },
      {
        id: '286',
        labels: {
          fa: 'کاکی',
          en: 'Kaki',
        },
      },
      {
        id: '287',
        labels: {
          fa: 'کلمه',
          en: 'Kalameh',
        },
      },
      {
        id: '288',
        labels: {
          fa: 'نخل تقی',
          en: 'Nakhl-e Taqi',
        },
      },
      {
        id: '289',
        labels: {
          fa: 'وحدتیه',
          en: 'Vahdattiyeh',
        },
      },
    ],
  },
  {
    id: 'tehran',
    labels: {
      fa: 'تهران',
      en: 'Tehran',
    },
    cities: [
      {
        id: '290',
        labels: {
          fa: 'ارجمند',
          en: 'Arjomand',
        },
      },
      {
        id: '291',
        labels: {
          fa: 'اسلامشهر',
          en: 'Eslamshahr',
        },
      },
      {
        id: '292',
        labels: {
          fa: 'اندیشه',
          en: 'Andisheh',
        },
      },
      {
        id: '293',
        labels: {
          fa: 'آبسرد',
          en: 'Absard',
        },
      },
      {
        id: '294',
        labels: {
          fa: 'آبعلی',
          en: 'Abali',
        },
      },
      {
        id: '295',
        labels: {
          fa: 'باغستان',
          en: 'Baghestan',
        },
      },
      {
        id: '296',
        labels: {
          fa: 'باقرشهر',
          en: 'Baqershahr',
        },
      },
      {
        id: '297',
        labels: {
          fa: 'بومهن',
          en: 'Bumahen',
        },
      },
      {
        id: '298',
        labels: {
          fa: 'پاکدشت',
          en: 'Pakdasht',
        },
      },
      {
        id: '299',
        labels: {
          fa: 'پردیس',
          en: 'Pardis',
        },
      },
      {
        id: '300',
        labels: {
          fa: 'پیشوا',
          en: 'Pishva',
        },
      },
      {
        id: '301',
        labels: {
          fa: 'تهران',
          en: 'Tehran',
        },
      },
      {
        id: '302',
        labels: {
          fa: 'جوادآباد',
          en: 'Javadiyeh',
        },
      },
      {
        id: '303',
        labels: {
          fa: 'چهاردانگه',
          en: 'Chahardangeh',
        },
      },
      {
        id: '304',
        labels: {
          fa: 'حسن آباد',
          en: 'Hasanabad',
        },
      },
      {
        id: '305',
        labels: {
          fa: 'دماوند',
          en: 'Damavand',
        },
      },
      {
        id: '306',
        labels: {
          fa: 'دیزین',
          en: 'Dizin',
        },
      },
      {
        id: '307',
        labels: {
          fa: 'شهر ری',
          en: 'Rey',
        },
      },
      {
        id: '308',
        labels: {
          fa: 'رباط کریم',
          en: 'Robat Karim',
        },
      },
      {
        id: '309',
        labels: {
          fa: 'رودهن',
          en: 'Rudehen',
        },
      },
      {
        id: '310',
        labels: {
          fa: 'شاهدشهر',
          en: 'Shahedshahr',
        },
      },
      {
        id: '311',
        labels: {
          fa: 'شریف آباد',
          en: 'Sharifabad',
        },
      },
      {
        id: '312',
        labels: {
          fa: 'شمشک',
          en: 'Shemshak',
        },
      },
      {
        id: '313',
        labels: {
          fa: 'شهریار',
          en: 'Shahriar',
        },
      },
      {
        id: '314',
        labels: {
          fa: 'صالح آباد',
          en: 'Salehabad',
        },
      },
      {
        id: '315',
        labels: {
          fa: 'صباشهر',
          en: 'Sabbashahr',
        },
      },
      {
        id: '316',
        labels: {
          fa: 'صفادشت',
          en: 'Shahrabad',
        },
      },
      {
        id: '317',
        labels: {
          fa: 'فردوسیه',
          en: 'Ferdowsiyeh',
        },
      },
      {
        id: '318',
        labels: {
          fa: 'فشم',
          en: 'Fesham',
        },
      },
      {
        id: '319',
        labels: {
          fa: 'فیروزکوه',
          en: 'Firuzkkuh',
        },
      },
      {
        id: '320',
        labels: {
          fa: 'قدس',
          en: 'Qods',
        },
      },
      {
        id: '321',
        labels: {
          fa: 'قرچک',
          en: 'Qarchak',
        },
      },
      {
        id: '322',
        labels: {
          fa: 'کهریزک',
          en: 'Kahrizak',
        },
      },
      {
        id: '323',
        labels: {
          fa: 'کیلان',
          en: 'Kilan',
        },
      },
      {
        id: '324',
        labels: {
          fa: 'گلستان',
          en: 'Golestan',
        },
      },
      {
        id: '325',
        labels: {
          fa: 'لواسان',
          en: 'Lavasan',
        },
      },
      {
        id: '326',
        labels: {
          fa: 'ملارد',
          en: 'Malard',
        },
      },
      {
        id: '327',
        labels: {
          fa: 'میگون',
          en: 'Meygun',
        },
      },
      {
        id: '328',
        labels: {
          fa: 'نسیم شهر',
          en: 'Nasim Shahr',
        },
      },
      {
        id: '329',
        labels: {
          fa: 'نصیرآباد',
          en: 'Nazarabad',
        },
      },
      {
        id: '330',
        labels: {
          fa: 'وحیدیه',
          en: 'Vahidiyeh',
        },
      },
      {
        id: '331',
        labels: {
          fa: 'ورامین',
          en: 'Varamin',
        },
      },
      {
        id: '1116',
        labels: {
          fa: 'پرند',
          en: 'Parand',
        },
      },
      {
        id: '1153',
        labels: {
          fa: 'قیامدشت',
          en: 'Shahrak-e Qiamdasht',
        },
      },
    ],
  },
  {
    id: 'chaharmahal-and-bakhtiari',
    labels: {
      fa: 'چهارمحال و بختیاری',
      en: 'Chaharmahal and Bakhtiari',
    },
    cities: [
      {
        id: '332',
        labels: {
          fa: 'اردل',
          en: 'Ardal',
        },
      },
      {
        id: '333',
        labels: {
          fa: 'آلونی',
          en: 'Aluni',
        },
      },
      {
        id: '334',
        labels: {
          fa: 'باباحیدر',
          en: 'Baba Heydar',
        },
      },
      {
        id: '335',
        labels: {
          fa: 'بروجن',
          en: 'Borujen',
        },
      },
      {
        id: '336',
        labels: {
          fa: 'بلداجی',
          en: 'Boldaji',
        },
      },
      {
        id: '337',
        labels: {
          fa: 'بن',
          en: 'Bon',
        },
      },
      {
        id: '338',
        labels: {
          fa: 'جونقان',
          en: 'Junqan',
        },
      },
      {
        id: '339',
        labels: {
          fa: 'چلگرد',
          en: 'Chelgerd',
        },
      },
      {
        id: '340',
        labels: {
          fa: 'سامان',
          en: 'Saman',
        },
      },
      {
        id: '341',
        labels: {
          fa: 'سفیددشت',
          en: 'Sefid Dasht',
        },
      },
      {
        id: '342',
        labels: {
          fa: 'سودجان',
          en: 'Sud Jan',
        },
      },
      {
        id: '343',
        labels: {
          fa: 'سورشجان',
          en: 'Sureshjan',
        },
      },
      {
        id: '344',
        labels: {
          fa: 'شلمزار',
          en: 'Shalamzar',
        },
      },
      {
        id: '345',
        labels: {
          fa: 'شهرکرد',
          en: 'Shahrekord',
        },
      },
      {
        id: '346',
        labels: {
          fa: 'طاقانک',
          en: 'Taqanak',
        },
      },
      {
        id: '347',
        labels: {
          fa: 'فارسان',
          en: 'Farsan',
        },
      },
      {
        id: '348',
        labels: {
          fa: 'فرادنبه',
          en: 'Faradonbeh',
        },
      },
      {
        id: '349',
        labels: {
          fa: 'فرخ شهر',
          en: 'Farrokh Shahr',
        },
      },
      {
        id: '350',
        labels: {
          fa: 'کیان',
          en: 'Shahr-e Kian',
        },
      },
      {
        id: '351',
        labels: {
          fa: 'گندمان',
          en: 'Gandoman',
        },
      },
      {
        id: '352',
        labels: {
          fa: 'گهرو',
          en: 'Gahru',
        },
      },
      {
        id: '353',
        labels: {
          fa: 'لردگان',
          en: 'Lordegan',
        },
      },
      {
        id: '354',
        labels: {
          fa: 'مال خلیفه',
          en: 'Mal Khalifeh',
        },
      },
      {
        id: '355',
        labels: {
          fa: 'ناغان',
          en: 'Naghan',
        },
      },
      {
        id: '356',
        labels: {
          fa: 'نافچ',
          en: 'Nafech',
        },
      },
      {
        id: '357',
        labels: {
          fa: 'نقنه',
          en: 'Naqneh',
        },
      },
      {
        id: '358',
        labels: {
          fa: 'هفشجان',
          en: 'Hafshejan',
        },
      },
    ],
  },
  {
    id: 'south-khorasan',
    labels: {
      fa: 'خراسان جنوبی',
      en: 'South Khorasan',
    },
    cities: [
      {
        id: '359',
        labels: {
          fa: 'ارسک',
          en: 'Eresk',
        },
      },
      {
        id: '360',
        labels: {
          fa: 'اسدیه',
          en: 'Asadiyeh',
        },
      },
      {
        id: '361',
        labels: {
          fa: 'اسفدن',
          en: 'Esfeden',
        },
      },
      {
        id: '362',
        labels: {
          fa: 'اسلامیه',
          en: 'Eslamiyeh',
        },
      },
      {
        id: '363',
        labels: {
          fa: 'آرین شهر',
          en: 'Arian Shahr',
        },
      },
      {
        id: '364',
        labels: {
          fa: 'آیسک',
          en: 'Ayask',
        },
      },
      {
        id: '365',
        labels: {
          fa: 'بشرویه',
          en: 'Boshruyeh',
        },
      },
      {
        id: '366',
        labels: {
          fa: 'بیرجند',
          en: 'Birjand',
        },
      },
      {
        id: '367',
        labels: {
          fa: 'حاجی آباد',
          en: 'Hajjiabad',
        },
      },
      {
        id: '368',
        labels: {
          fa: 'خضری دشت بیاض',
          en: 'Khezri Dasht Beyaz',
        },
      },
      {
        id: '369',
        labels: {
          fa: 'خوسف',
          en: 'Khusf',
        },
      },
      {
        id: '370',
        labels: {
          fa: 'زهان',
          en: 'Zohan',
        },
      },
      {
        id: '371',
        labels: {
          fa: 'سرایان',
          en: 'Sarayan',
        },
      },
      {
        id: '372',
        labels: {
          fa: 'سربیشه',
          en: 'Sarbisheh',
        },
      },
      {
        id: '373',
        labels: {
          fa: 'سه قلعه',
          en: "Seh Qal'eh",
        },
      },
      {
        id: '374',
        labels: {
          fa: 'شوسف',
          en: 'Shusef',
        },
      },
      {
        id: '375',
        labels: {
          fa: 'طبس ',
          en: 'Tabas',
        },
      },
      {
        id: '376',
        labels: {
          fa: 'فردوس',
          en: 'Ferdows',
        },
      },
      {
        id: '377',
        labels: {
          fa: 'قاین',
          en: "Qa'en",
        },
      },
      {
        id: '378',
        labels: {
          fa: 'قهستان',
          en: 'Qohestan',
        },
      },
      {
        id: '379',
        labels: {
          fa: 'محمدشهر',
          en: 'Mohammadshahr',
        },
      },
      {
        id: '380',
        labels: {
          fa: 'مود',
          en: 'Mud',
        },
      },
      {
        id: '381',
        labels: {
          fa: 'نهبندان',
          en: 'Nehbandan',
        },
      },
      {
        id: '382',
        labels: {
          fa: 'نیمبلوک',
          en: 'Nimbeluk',
        },
      },
    ],
  },
  {
    id: 'razavi-khorasan',
    labels: {
      fa: 'خراسان رضوی',
      en: 'Razavi Khorasan',
    },
    cities: [
      {
        id: '383',
        labels: {
          fa: 'احمدآباد صولت',
          en: 'Ahmadabad-e Sowlat',
        },
      },
      {
        id: '384',
        labels: {
          fa: 'انابد',
          en: 'Anabad',
        },
      },
      {
        id: '385',
        labels: {
          fa: 'باجگیران',
          en: 'Bajgiran',
        },
      },
      {
        id: '386',
        labels: {
          fa: 'باخرز',
          en: 'Bakharz',
        },
      },
      {
        id: '387',
        labels: {
          fa: 'بار',
          en: 'Bar',
        },
      },
      {
        id: '388',
        labels: {
          fa: 'بایگ',
          en: 'Bayg',
        },
      },
      {
        id: '389',
        labels: {
          fa: 'بجستان',
          en: 'Bajestan',
        },
      },
      {
        id: '390',
        labels: {
          fa: 'بردسکن',
          en: 'Bardaskan',
        },
      },
      {
        id: '391',
        labels: {
          fa: 'بیدخت',
          en: 'Bidokht',
        },
      },
      {
        id: '392',
        labels: {
          fa: 'تایباد',
          en: 'Taybad',
        },
      },
      {
        id: '393',
        labels: {
          fa: 'تربت جام',
          en: 'Torbat Jam',
        },
      },
      {
        id: '394',
        labels: {
          fa: 'تربت حیدریه',
          en: 'Torbat Heydarieh',
        },
      },
      {
        id: '395',
        labels: {
          fa: 'جغتای',
          en: 'Joghatai',
        },
      },
      {
        id: '396',
        labels: {
          fa: 'جنگل',
          en: 'Jangal',
        },
      },
      {
        id: '397',
        labels: {
          fa: 'چاپشلو',
          en: 'Chapeshlu',
        },
      },
      {
        id: '398',
        labels: {
          fa: 'چکنه',
          en: 'Chekneh',
        },
      },
      {
        id: '399',
        labels: {
          fa: 'چناران',
          en: 'Chenaran',
        },
      },
      {
        id: '400',
        labels: {
          fa: 'خرو',
          en: 'Kharu',
        },
      },
      {
        id: '401',
        labels: {
          fa: 'خلیل آباد',
          en: 'Khalilabad',
        },
      },
      {
        id: '402',
        labels: {
          fa: 'خواف',
          en: 'Khvaf',
        },
      },
      {
        id: '403',
        labels: {
          fa: 'داورزن',
          en: 'Davarzan',
        },
      },
      {
        id: '404',
        labels: {
          fa: 'درگز',
          en: 'Dargaz',
        },
      },
      {
        id: '405',
        labels: {
          fa: 'در رود',
          en: 'Dorud',
        },
      },
      {
        id: '406',
        labels: {
          fa: 'دولت آباد',
          en: 'Dowlatabad',
        },
      },
      {
        id: '407',
        labels: {
          fa: 'رباط سنگ',
          en: 'Robat-e Sang',
        },
      },
      {
        id: '408',
        labels: {
          fa: 'رشتخوار',
          en: 'Rashtkhvar',
        },
      },
      {
        id: '409',
        labels: {
          fa: 'رضویه',
          en: 'Razaviyeh',
        },
      },
      {
        id: '410',
        labels: {
          fa: 'روداب',
          en: "Qal'eh Now Rudab",
        },
      },
      {
        id: '411',
        labels: {
          fa: 'ریوش',
          en: 'Rivash',
        },
      },
      {
        id: '412',
        labels: {
          fa: 'سبزوار',
          en: 'Sabzevar',
        },
      },
      {
        id: '413',
        labels: {
          fa: 'سرخس',
          en: 'Sarakhs',
        },
      },
      {
        id: '414',
        labels: {
          fa: 'سفیدسنگ',
          en: 'Sefid Sang',
        },
      },
      {
        id: '415',
        labels: {
          fa: 'سلامی',
          en: 'Salameh',
        },
      },
      {
        id: '416',
        labels: {
          fa: 'سلطان آباد',
          en: 'Soltanabad',
        },
      },
      {
        id: '417',
        labels: {
          fa: 'سنگان',
          en: 'Sangan',
        },
      },
      {
        id: '418',
        labels: {
          fa: 'شادمهر',
          en: 'Shadmehr',
        },
      },
      {
        id: '419',
        labels: {
          fa: 'شاندیز',
          en: 'Shandiz',
        },
      },
      {
        id: '420',
        labels: {
          fa: 'ششتمد',
          en: 'Sheshtamad',
        },
      },
      {
        id: '421',
        labels: {
          fa: 'شهرآباد',
          en: 'Shahrabad',
        },
      },
      {
        id: '422',
        labels: {
          fa: 'شهرزو',
          en: 'Shahr-e Zow',
        },
      },
      {
        id: '423',
        labels: {
          fa: 'صالح آباد',
          en: 'Salehabad',
        },
      },
      {
        id: '424',
        labels: {
          fa: 'طرقبه',
          en: 'Torqabeh',
        },
      },
      {
        id: '425',
        labels: {
          fa: 'عشق آباد',
          en: "'Eshqabad",
        },
      },
      {
        id: '426',
        labels: {
          fa: 'فرهادگرد',
          en: 'Farhadgerd',
        },
      },
      {
        id: '427',
        labels: {
          fa: 'فریمان',
          en: 'Fariman',
        },
      },
      {
        id: '428',
        labels: {
          fa: 'فیروزه',
          en: 'Firuzeh',
        },
      },
      {
        id: '429',
        labels: {
          fa: 'فیض آباد',
          en: 'Feyzabad',
        },
      },
      {
        id: '430',
        labels: {
          fa: 'قاسم آباد',
          en: 'Qasemabad',
        },
      },
      {
        id: '431',
        labels: {
          fa: 'قدمگاه',
          en: 'Qadamgah',
        },
      },
      {
        id: '432',
        labels: {
          fa: 'قلندرآباد',
          en: 'Qalandarabad',
        },
      },
      {
        id: '433',
        labels: {
          fa: 'قوچان',
          en: 'Quchan',
        },
      },
      {
        id: '434',
        labels: {
          fa: 'کاخک',
          en: 'Kakhk',
        },
      },
      {
        id: '435',
        labels: {
          fa: 'کاریز',
          en: 'Kariz',
        },
      },
      {
        id: '436',
        labels: {
          fa: 'کاشمر',
          en: 'Kashmar',
        },
      },
      {
        id: '437',
        labels: {
          fa: 'کدکن',
          en: 'Kadkan',
        },
      },
      {
        id: '438',
        labels: {
          fa: 'کلات',
          en: 'Kalat',
        },
      },
      {
        id: '439',
        labels: {
          fa: 'کندر',
          en: 'Kondor',
        },
      },
      {
        id: '440',
        labels: {
          fa: 'گلمکان',
          en: 'Golmakan',
        },
      },
      {
        id: '441',
        labels: {
          fa: 'گناباد',
          en: 'Gonabad',
        },
      },
      {
        id: '442',
        labels: {
          fa: 'لطف آباد',
          en: 'Lotfabad',
        },
      },
      {
        id: '443',
        labels: {
          fa: 'مزدآوند',
          en: 'Mazdavand',
        },
      },
      {
        id: '444',
        labels: {
          fa: 'مشهد',
          en: 'Mashhad',
        },
      },
      {
        id: '445',
        labels: {
          fa: 'ملک آباد',
          en: 'Malekabad',
        },
      },
      {
        id: '446',
        labels: {
          fa: 'نشتیفان',
          en: 'Nashtifan',
        },
      },
      {
        id: '447',
        labels: {
          fa: 'نصرآباد',
          en: 'Nasrabad',
        },
      },
      {
        id: '448',
        labels: {
          fa: 'نقاب',
          en: 'Neqab',
        },
      },
      {
        id: '449',
        labels: {
          fa: 'نوخندان',
          en: 'Nowkhandan',
        },
      },
      {
        id: '450',
        labels: {
          fa: 'نیشابور',
          en: 'Nishapur',
        },
      },
      {
        id: '451',
        labels: {
          fa: 'نیل شهر',
          en: 'Nilshahr',
        },
      },
      {
        id: '452',
        labels: {
          fa: 'همت آباد',
          en: 'Hemmatabad',
        },
      },
      {
        id: '453',
        labels: {
          fa: 'یونسی',
          en: 'Yunesi',
        },
      },
      {
        id: '1150',
        labels: {
          fa: 'گلبهار',
          en: 'Golbahar',
        },
      },
      {
        id: '1155',
        labels: {
          fa: 'بینالود',
          en: 'Binaloud',
        },
      },
    ],
  },
  {
    id: 'north-khorasan',
    labels: {
      fa: 'خراسان شمالی',
      en: 'North Khorasan',
    },
    cities: [
      {
        id: '454',
        labels: {
          fa: 'اسفراین',
          en: 'Esfarayen',
        },
      },
      {
        id: '455',
        labels: {
          fa: 'ایور',
          en: 'Ivar',
        },
      },
      {
        id: '456',
        labels: {
          fa: 'آشخانه',
          en: 'Ashkhaneh',
        },
      },
      {
        id: '457',
        labels: {
          fa: 'بجنورد',
          en: 'Bojnord',
        },
      },
      {
        id: '458',
        labels: {
          fa: 'پیش قلعه',
          en: "Pish Qal'eh",
        },
      },
      {
        id: '459',
        labels: {
          fa: 'تیتکانلو',
          en: 'Titkanlu',
        },
      },
      {
        id: '460',
        labels: {
          fa: 'جاجرم',
          en: 'Jajrom',
        },
      },
      {
        id: '461',
        labels: {
          fa: 'حصارگرمخان',
          en: 'Hesar-e Garmkhan',
        },
      },
      {
        id: '462',
        labels: {
          fa: 'درق',
          en: 'Daraq',
        },
      },
      {
        id: '463',
        labels: {
          fa: 'راز',
          en: 'Raz',
        },
      },
      {
        id: '464',
        labels: {
          fa: 'سنخواست',
          en: 'Sankhvast',
        },
      },
      {
        id: '465',
        labels: {
          fa: 'شوقان',
          en: 'Showqan',
        },
      },
      {
        id: '466',
        labels: {
          fa: 'شیروان',
          en: 'Shirvan',
        },
      },
      {
        id: '467',
        labels: {
          fa: 'صفی آباد',
          en: 'Safiabad',
        },
      },
      {
        id: '468',
        labels: {
          fa: 'فاروج',
          en: 'Faruj',
        },
      },
      {
        id: '469',
        labels: {
          fa: 'قاضی',
          en: 'Qazi',
        },
      },
      {
        id: '470',
        labels: {
          fa: 'گرمه',
          en: 'Garmeh',
        },
      },
      {
        id: '471',
        labels: {
          fa: 'لوجلی',
          en: 'Lujali',
        },
      },
    ],
  },
  {
    id: 'khuzestan',
    labels: {
      fa: 'خوزستان',
      en: 'Khuzestan',
    },
    cities: [
      {
        id: '472',
        labels: {
          fa: 'اروندکنار',
          en: 'Arvandkenar',
        },
      },
      {
        id: '473',
        labels: {
          fa: 'الوان',
          en: "Boneh-ye 'Alvan",
        },
      },
      {
        id: '474',
        labels: {
          fa: 'امیدیه',
          en: 'Omidiyeh',
        },
      },
      {
        id: '475',
        labels: {
          fa: 'اندیمشک',
          en: 'Andimeshk',
        },
      },
      {
        id: '476',
        labels: {
          fa: 'اهواز',
          en: 'Ahvaz',
        },
      },
      {
        id: '477',
        labels: {
          fa: 'ایذه',
          en: 'Izeh',
        },
      },
      {
        id: '478',
        labels: {
          fa: 'آبادان',
          en: 'Abadan',
        },
      },
      {
        id: '479',
        labels: {
          fa: 'آغاجاری',
          en: 'Aghajari',
        },
      },
      {
        id: '480',
        labels: {
          fa: 'باغ ملک',
          en: 'Bagh Malek',
        },
      },
      {
        id: '481',
        labels: {
          fa: 'بستان',
          en: 'Bostan',
        },
      },
      {
        id: '482',
        labels: {
          fa: 'بندرامام خمینی',
          en: 'Bandar Imam Khomeini',
        },
      },
      {
        id: '483',
        labels: {
          fa: 'بندرماهشهر',
          en: 'Bandar Mahshahr',
        },
      },
      {
        id: '484',
        labels: {
          fa: 'بهبهان',
          en: 'Behbahan',
        },
      },
      {
        id: '485',
        labels: {
          fa: 'ترکالکی',
          en: 'Torkalaki',
        },
      },
      {
        id: '486',
        labels: {
          fa: 'جایزان',
          en: 'Jayezan',
        },
      },
      {
        id: '487',
        labels: {
          fa: 'چمران',
          en: 'Shahrak-e Shahid Chamran',
        },
      },
      {
        id: '488',
        labels: {
          fa: 'چویبده',
          en: "Chu'ebdeh-ye Yek",
        },
      },
      {
        id: '489',
        labels: {
          fa: 'حر',
          en: 'Horr-e Riahi',
        },
      },
      {
        id: '490',
        labels: {
          fa: 'حسینیه',
          en: 'Hoseyniyeh',
        },
      },
      {
        id: '491',
        labels: {
          fa: 'حمزه',
          en: 'Shahrak-e Hamzeh',
        },
      },
      {
        id: '492',
        labels: {
          fa: 'حمیدیه',
          en: 'Hamidiyeh',
        },
      },
      {
        id: '493',
        labels: {
          fa: 'خرمشهر',
          en: 'Khorramshahr',
        },
      },
      {
        id: '494',
        labels: {
          fa: 'دارخوین',
          en: 'Darkhovin',
        },
      },
      {
        id: '495',
        labels: {
          fa: 'دزآب',
          en: 'Dezab',
        },
      },
      {
        id: '496',
        labels: {
          fa: 'دزفول',
          en: 'Dezful',
        },
      },
      {
        id: '497',
        labels: {
          fa: 'دهدز',
          en: 'Dehdez',
        },
      },
      {
        id: '498',
        labels: {
          fa: 'رامشیر',
          en: 'Ramshir',
        },
      },
      {
        id: '499',
        labels: {
          fa: 'رامهرمز',
          en: 'Ramhormoz',
        },
      },
      {
        id: '500',
        labels: {
          fa: 'رفیع',
          en: "Rafi'",
        },
      },
      {
        id: '501',
        labels: {
          fa: 'زهره',
          en: 'Zahreh',
        },
      },
      {
        id: '502',
        labels: {
          fa: 'سالند',
          en: 'Saland',
        },
      },
      {
        id: '503',
        labels: {
          fa: 'سردشت',
          en: 'Sardasht',
        },
      },
      {
        id: '504',
        labels: {
          fa: 'سوسنگرد',
          en: 'Susangerd',
        },
      },
      {
        id: '505',
        labels: {
          fa: 'شادگان',
          en: 'Shadegan',
        },
      },
      {
        id: '506',
        labels: {
          fa: 'شاوور',
          en: "Moslem Ebn-e 'Aqil",
        },
      },
      {
        id: '507',
        labels: {
          fa: 'شرافت',
          en: 'Sharafat',
        },
      },
      {
        id: '508',
        labels: {
          fa: 'شوش',
          en: 'Shush',
        },
      },
      {
        id: '509',
        labels: {
          fa: 'شوشتر',
          en: 'Shushtar',
        },
      },
      {
        id: '510',
        labels: {
          fa: 'شیبان',
          en: 'Sheyban',
        },
      },
      {
        id: '511',
        labels: {
          fa: 'صالح شهر',
          en: 'Saleh Shahr',
        },
      },
      {
        id: '512',
        labels: {
          fa: 'صفی آباد',
          en: 'Safiabad',
        },
      },
      {
        id: '513',
        labels: {
          fa: 'صیدون',
          en: 'Seydun',
        },
      },
      {
        id: '514',
        labels: {
          fa: 'قلعه تل',
          en: "Qal'eh Tall",
        },
      },
      {
        id: '515',
        labels: {
          fa: 'قلعه خواجه',
          en: "Qal'eh-ye Khvajeh",
        },
      },
      {
        id: '516',
        labels: {
          fa: 'گتوند',
          en: 'Gotvand',
        },
      },
      {
        id: '517',
        labels: {
          fa: 'لالی',
          en: 'Lali',
        },
      },
      {
        id: '518',
        labels: {
          fa: 'مسجدسلیمان',
          en: 'Masjed Soleyman',
        },
      },
      {
        id: '520',
        labels: {
          fa: 'ملاثانی',
          en: 'Mollasani',
        },
      },
      {
        id: '521',
        labels: {
          fa: 'میانرود',
          en: 'Shahrak-e Taleqani',
        },
      },
      {
        id: '522',
        labels: {
          fa: 'مینوشهر',
          en: 'Minushahr',
        },
      },
      {
        id: '523',
        labels: {
          fa: 'هفتگل',
          en: 'Haftkel',
        },
      },
      {
        id: '524',
        labels: {
          fa: 'هندیجان',
          en: 'Hendijan',
        },
      },
      {
        id: '525',
        labels: {
          fa: 'هویزه',
          en: 'Hoveyzeh',
        },
      },
      {
        id: '526',
        labels: {
          fa: 'ویس',
          en: 'Veys',
        },
      },
    ],
  },
  {
    id: 'zanjan',
    labels: {
      fa: 'زنجان',
      en: 'Zanjan',
    },
    cities: [
      {
        id: '527',
        labels: {
          fa: 'ابهر',
          en: 'Abhar',
        },
      },
      {
        id: '528',
        labels: {
          fa: 'ارمغان خانه',
          en: 'Armaghankhaneh',
        },
      },
      {
        id: '529',
        labels: {
          fa: 'آب بر',
          en: 'Ab Bar',
        },
      },
      {
        id: '530',
        labels: {
          fa: 'چورزق',
          en: 'Churzaq',
        },
      },
      {
        id: '531',
        labels: {
          fa: 'حلب',
          en: 'Halab',
        },
      },
      {
        id: '532',
        labels: {
          fa: 'خرمدره',
          en: 'Khorramdarreh',
        },
      },
      {
        id: '533',
        labels: {
          fa: 'دندی',
          en: 'Dandi',
        },
      },
      {
        id: '534',
        labels: {
          fa: 'زرین آباد',
          en: 'Zarrinabad',
        },
      },
      {
        id: '535',
        labels: {
          fa: 'زرین رود',
          en: 'Zarrin Rud',
        },
      },
      {
        id: '536',
        labels: {
          fa: 'زنجان',
          en: 'Zanjan',
        },
      },
      {
        id: '537',
        labels: {
          fa: 'سجاس',
          en: 'Sojas',
        },
      },
      {
        id: '538',
        labels: {
          fa: 'سلطانیه',
          en: 'Soltaniyeh',
        },
      },
      {
        id: '539',
        labels: {
          fa: 'سهرورد',
          en: 'Qareh Qush',
        },
      },
      {
        id: '540',
        labels: {
          fa: 'صائین قلعه',
          en: "Sa'in Qal'eh",
        },
      },
      {
        id: '541',
        labels: {
          fa: 'قیدار',
          en: 'Qeydar',
        },
      },
      {
        id: '542',
        labels: {
          fa: 'گرماب',
          en: 'Garm Ab',
        },
      },
      {
        id: '543',
        labels: {
          fa: 'ماه نشان',
          en: 'Mahneshan',
        },
      },
      {
        id: '544',
        labels: {
          fa: 'هیدج',
          en: 'Hidaj',
        },
      },
    ],
  },
  {
    id: 'semnan',
    labels: {
      fa: 'سمنان',
      en: 'Semnan',
    },
    cities: [
      {
        id: '545',
        labels: {
          fa: 'امیریه',
          en: 'Amiriyeh',
        },
      },
      {
        id: '546',
        labels: {
          fa: 'ایوانکی',
          en: 'Ivanki',
        },
      },
      {
        id: '547',
        labels: {
          fa: 'آرادان',
          en: 'Aradan',
        },
      },
      {
        id: '548',
        labels: {
          fa: 'بسطام',
          en: 'Bastam',
        },
      },
      {
        id: '549',
        labels: {
          fa: 'بیارجمند',
          en: 'Biarjomand',
        },
      },
      {
        id: '550',
        labels: {
          fa: 'دامغان',
          en: 'Damghan',
        },
      },
      {
        id: '551',
        labels: {
          fa: 'درجزین',
          en: 'Darjazin',
        },
      },
      {
        id: '552',
        labels: {
          fa: 'دیباج',
          en: 'Dibaj',
        },
      },
      {
        id: '553',
        labels: {
          fa: 'سرخه',
          en: 'Sorkheh',
        },
      },
      {
        id: '554',
        labels: {
          fa: 'سمنان',
          en: 'Semnan',
        },
      },
      {
        id: '555',
        labels: {
          fa: 'شاهرود',
          en: 'Shahrud',
        },
      },
      {
        id: '556',
        labels: {
          fa: 'شهمیرزاد',
          en: 'Shahmirzad',
        },
      },
      {
        id: '557',
        labels: {
          fa: 'کلاته خیج',
          en: 'Kalateh-ye Khij',
        },
      },
      {
        id: '558',
        labels: {
          fa: 'گرمسار',
          en: 'Garmsar',
        },
      },
      {
        id: '559',
        labels: {
          fa: 'مجن',
          en: 'Mojen',
        },
      },
      {
        id: '560',
        labels: {
          fa: 'مهدی شهر',
          en: 'Mehdishahr',
        },
      },
      {
        id: '561',
        labels: {
          fa: 'میامی',
          en: 'Meyami',
        },
      },
    ],
  },
  {
    id: 'sistan-and-baluchestan',
    labels: {
      fa: 'سیستان و بلوچستان',
      en: 'Sistan and Baluchestan',
    },
    cities: [
      {
        id: '562',
        labels: {
          fa: 'ادیمی',
          en: 'Adimi',
        },
      },
      {
        id: '563',
        labels: {
          fa: 'اسپکه',
          en: 'Espakeh',
        },
      },
      {
        id: '564',
        labels: {
          fa: 'ایرانشهر',
          en: 'Iranshahr',
        },
      },
      {
        id: '565',
        labels: {
          fa: 'بزمان',
          en: 'Bazman',
        },
      },
      {
        id: '566',
        labels: {
          fa: 'بمپور',
          en: 'Bampur',
        },
      },
      {
        id: '567',
        labels: {
          fa: 'بنت',
          en: 'Bent',
        },
      },
      {
        id: '568',
        labels: {
          fa: 'بنجار',
          en: 'Bonjar',
        },
      },
      {
        id: '569',
        labels: {
          fa: 'پیشین',
          en: 'Pishin',
        },
      },
      {
        id: '570',
        labels: {
          fa: 'جالق',
          en: 'Jaleq',
        },
      },
      {
        id: '571',
        labels: {
          fa: 'چابهار',
          en: 'Chabahar',
        },
      },
      {
        id: '572',
        labels: {
          fa: 'خاش',
          en: 'Khash',
        },
      },
      {
        id: '573',
        labels: {
          fa: 'دوست محمد',
          en: 'Dust Mohammad',
        },
      },
      {
        id: '574',
        labels: {
          fa: 'راسک',
          en: 'Rasak',
        },
      },
      {
        id: '575',
        labels: {
          fa: 'زابل',
          en: 'Zabol',
        },
      },
      {
        id: '576',
        labels: {
          fa: 'زابلی',
          en: 'Zaboli',
        },
      },
      {
        id: '577',
        labels: {
          fa: 'زاهدان',
          en: 'Zahedan',
        },
      },
      {
        id: '578',
        labels: {
          fa: 'زهک',
          en: 'Zehak',
        },
      },
      {
        id: '579',
        labels: {
          fa: 'سراوان',
          en: 'Saravan',
        },
      },
      {
        id: '580',
        labels: {
          fa: 'سرباز',
          en: 'Sarbaz',
        },
      },
      {
        id: '581',
        labels: {
          fa: 'سوران',
          en: 'Suran',
        },
      },
      {
        id: '582',
        labels: {
          fa: 'سیرکان',
          en: 'Sirkan',
        },
      },
      {
        id: '583',
        labels: {
          fa: 'علی اکبر',
          en: 'Ali Akbar',
        },
      },
      {
        id: '584',
        labels: {
          fa: 'فنوج',
          en: 'Fanuj',
        },
      },
      {
        id: '585',
        labels: {
          fa: 'قصرقند',
          en: 'Qasr-e Qand',
        },
      },
      {
        id: '586',
        labels: {
          fa: 'کنارک',
          en: 'Konarak',
        },
      },
      {
        id: '587',
        labels: {
          fa: 'گشت',
          en: 'Gosht',
        },
      },
      {
        id: '588',
        labels: {
          fa: 'گلمورتی',
          en: 'Dalgan',
        },
      },
      {
        id: '589',
        labels: {
          fa: 'محمدان',
          en: 'Mohammadan',
        },
      },
      {
        id: '590',
        labels: {
          fa: 'محمدآباد',
          en: 'Mohammadabad',
        },
      },
      {
        id: '591',
        labels: {
          fa: 'محمدی',
          en: 'Mohammadi',
        },
      },
      {
        id: '592',
        labels: {
          fa: 'میرجاوه',
          en: 'Mirjaveh',
        },
      },
      {
        id: '593',
        labels: {
          fa: 'نصرت آباد',
          en: 'Nosratabad',
        },
      },
      {
        id: '594',
        labels: {
          fa: 'نگور',
          en: 'Negur',
        },
      },
      {
        id: '595',
        labels: {
          fa: 'نوک آباد',
          en: 'Nukabad',
        },
      },
      {
        id: '596',
        labels: {
          fa: 'نیک شهر',
          en: 'Nik Shahr',
        },
      },
      {
        id: '597',
        labels: {
          fa: 'هیدوچ',
          en: 'Hiduj',
        },
      },
    ],
  },
  {
    id: 'fars',
    labels: {
      fa: 'فارس',
      en: 'Fars',
    },
    cities: [
      {
        id: '598',
        labels: {
          fa: 'اردکان',
          en: 'Ardakan',
        },
      },
      {
        id: '599',
        labels: {
          fa: 'ارسنجان',
          en: 'Arsanjan',
        },
      },
      {
        id: '600',
        labels: {
          fa: 'استهبان',
          en: 'Estahban',
        },
      },
      {
        id: '601',
        labels: {
          fa: 'اشکنان',
          en: 'Eshkanan',
        },
      },
      {
        id: '602',
        labels: {
          fa: 'افزر',
          en: 'Efzar',
        },
      },
      {
        id: '603',
        labels: {
          fa: 'اقلید',
          en: 'Eqlid',
        },
      },
      {
        id: '604',
        labels: {
          fa: 'امام شهر',
          en: 'Shahrak-e Emam',
        },
      },
      {
        id: '605',
        labels: {
          fa: 'اهل',
          en: 'Ahl',
        },
      },
      {
        id: '606',
        labels: {
          fa: 'اوز',
          en: 'Evaz',
        },
      },
      {
        id: '607',
        labels: {
          fa: 'ایج',
          en: 'Ij',
        },
      },
      {
        id: '608',
        labels: {
          fa: 'ایزدخواست',
          en: 'Izad Khvast',
        },
      },
      {
        id: '609',
        labels: {
          fa: 'آباده',
          en: 'Abadeh',
        },
      },
      {
        id: '610',
        labels: {
          fa: 'آباده طشک',
          en: 'Abadeh-ye Tashk',
        },
      },
      {
        id: '611',
        labels: {
          fa: 'باب انار',
          en: 'Bab Anar',
        },
      },
      {
        id: '612',
        labels: {
          fa: 'بالاده',
          en: 'Bala Deh',
        },
      },
      {
        id: '613',
        labels: {
          fa: 'بنارویه',
          en: 'Banaruyeh',
        },
      },
      {
        id: '614',
        labels: {
          fa: 'بهمن',
          en: 'Bahman',
        },
      },
      {
        id: '615',
        labels: {
          fa: 'بوانات',
          en: 'Bavanat',
        },
      },
      {
        id: '616',
        labels: {
          fa: 'بیرم',
          en: 'Beyram',
        },
      },
      {
        id: '617',
        labels: {
          fa: 'بیضا',
          en: 'Tall Beyza',
        },
      },
      {
        id: '618',
        labels: {
          fa: 'جنت شهر',
          en: 'Jannat Shahr',
        },
      },
      {
        id: '619',
        labels: {
          fa: 'جهرم',
          en: 'Jahrom',
        },
      },
      {
        id: '620',
        labels: {
          fa: 'جویم',
          en: 'Juyom',
        },
      },
      {
        id: '621',
        labels: {
          fa: 'زرین دشت',
          en: 'Zarrin Dasht',
        },
      },
      {
        id: '622',
        labels: {
          fa: 'حسن آباد',
          en: 'Hasanabad',
        },
      },
      {
        id: '623',
        labels: {
          fa: 'خان زنیان',
          en: 'Khaneh Zenyan',
        },
      },
      {
        id: '624',
        labels: {
          fa: 'خاوران',
          en: 'Khavaran',
        },
      },
      {
        id: '625',
        labels: {
          fa: 'خرامه',
          en: 'Kharameh',
        },
      },
      {
        id: '626',
        labels: {
          fa: 'خشت',
          en: 'Khasht',
        },
      },
      {
        id: '627',
        labels: {
          fa: 'خنج',
          en: 'Khonj',
        },
      },
      {
        id: '628',
        labels: {
          fa: 'خور',
          en: 'Khur',
        },
      },
      {
        id: '629',
        labels: {
          fa: 'داراب',
          en: 'Darab',
        },
      },
      {
        id: '630',
        labels: {
          fa: 'داریان',
          en: 'Darian',
        },
      },
      {
        id: '631',
        labels: {
          fa: 'دبیران',
          en: 'Dabiran',
        },
      },
      {
        id: '632',
        labels: {
          fa: 'دژکرد',
          en: 'Deh-e now',
        },
      },
      {
        id: '633',
        labels: {
          fa: 'دهرم',
          en: 'Dehram',
        },
      },
      {
        id: '634',
        labels: {
          fa: 'دوبرجی',
          en: 'Do Borji',
        },
      },
      {
        id: '635',
        labels: {
          fa: 'رامجرد',
          en: 'Ramjerdamjerdi',
        },
      },
      {
        id: '636',
        labels: {
          fa: 'رونیز',
          en: "Rowniz-e 'Olya",
        },
      },
      {
        id: '637',
        labels: {
          fa: 'زاهدشهر',
          en: 'Zahed Shahr',
        },
      },
      {
        id: '638',
        labels: {
          fa: 'زرقان',
          en: 'Zarqan',
        },
      },
      {
        id: '639',
        labels: {
          fa: 'سده',
          en: 'Sedeh',
        },
      },
      {
        id: '640',
        labels: {
          fa: 'سروستان',
          en: 'Sarvestan',
        },
      },
      {
        id: '641',
        labels: {
          fa: 'سعادت شهر',
          en: "Sa'adat Shahr",
        },
      },
      {
        id: '642',
        labels: {
          fa: 'سورمق',
          en: 'Surmaq',
        },
      },
      {
        id: '643',
        labels: {
          fa: 'سیدان',
          en: 'Seyyedan',
        },
      },
      {
        id: '644',
        labels: {
          fa: 'ششده',
          en: 'Sheshdeh',
        },
      },
      {
        id: '645',
        labels: {
          fa: 'شهرپیر',
          en: 'Shahr-e Pir',
        },
      },
      {
        id: '646',
        labels: {
          fa: 'شهرصدرا',
          en: 'Sadra, shiraz, iran',
        },
      },
      {
        id: '647',
        labels: {
          fa: 'شیراز',
          en: 'Shiraz',
        },
      },
      {
        id: '648',
        labels: {
          fa: 'صغاد',
          en: 'Soghad',
        },
      },
      {
        id: '649',
        labels: {
          fa: 'صفاشهر',
          en: 'Safashahr',
        },
      },
      {
        id: '650',
        labels: {
          fa: 'علامرودشت',
          en: 'Alamarvdasht',
        },
      },
      {
        id: '651',
        labels: {
          fa: 'فدامی',
          en: 'Fadami',
        },
      },
      {
        id: '652',
        labels: {
          fa: 'فراشبند',
          en: 'Farashband',
        },
      },
      {
        id: '653',
        labels: {
          fa: 'فسا',
          en: 'Fasa',
        },
      },
      {
        id: '654',
        labels: {
          fa: 'فیروزآباد',
          en: 'Firuzabad',
        },
      },
      {
        id: '655',
        labels: {
          fa: 'قائمیه',
          en: "Qa'emiyeh",
        },
      },
      {
        id: '656',
        labels: {
          fa: 'قادرآباد',
          en: 'Qaderabad',
        },
      },
      {
        id: '657',
        labels: {
          fa: 'قطب آباد',
          en: 'Qotbabad',
        },
      },
      {
        id: '658',
        labels: {
          fa: 'قطرویه',
          en: 'Qatruyeh',
        },
      },
      {
        id: '659',
        labels: {
          fa: 'قیر',
          en: 'Qir',
        },
      },
      {
        id: '660',
        labels: {
          fa: 'کارزین (فتح آباد)',
          en: 'Karzin',
        },
      },
      {
        id: '661',
        labels: {
          fa: 'کازرون',
          en: 'Kazerun',
        },
      },
      {
        id: '662',
        labels: {
          fa: 'کامفیروز',
          en: 'Kamfiruz',
        },
      },
      {
        id: '663',
        labels: {
          fa: 'کره ای',
          en: "Korreh'i",
        },
      },
      {
        id: '664',
        labels: {
          fa: 'کنارتخته',
          en: 'Konar Takhteh',
        },
      },
      {
        id: '665',
        labels: {
          fa: 'کوار',
          en: 'Kavar',
        },
      },
      {
        id: '666',
        labels: {
          fa: 'گراش',
          en: 'Gerash',
        },
      },
      {
        id: '667',
        labels: {
          fa: 'گله دار',
          en: 'Galleh Dar',
        },
      },
      {
        id: '668',
        labels: {
          fa: 'لار',
          en: 'Lar',
        },
      },
      {
        id: '669',
        labels: {
          fa: 'لامرد',
          en: 'Lamerd',
        },
      },
      {
        id: '670',
        labels: {
          fa: 'لپویی',
          en: 'Lapui',
        },
      },
      {
        id: '671',
        labels: {
          fa: 'لطیفی',
          en: 'Latifi',
        },
      },
      {
        id: '672',
        labels: {
          fa: 'مبارک آباددیز',
          en: 'Mobarakabad',
        },
      },
      {
        id: '673',
        labels: {
          fa: 'مرودشت',
          en: 'Marvdasht',
        },
      },
      {
        id: '674',
        labels: {
          fa: 'مشکان',
          en: 'Moshkan',
        },
      },
      {
        id: '675',
        labels: {
          fa: 'مصیری',
          en: 'Masiri',
        },
      },
      {
        id: '676',
        labels: {
          fa: 'مهر',
          en: 'Mohr',
        },
      },
      {
        id: '677',
        labels: {
          fa: 'میمند',
          en: 'Meymand',
        },
      },
      {
        id: '678',
        labels: {
          fa: 'نوبندگان',
          en: 'Now Bandegan',
        },
      },
      {
        id: '679',
        labels: {
          fa: 'نوجین',
          en: 'Nowjeyn',
        },
      },
      {
        id: '680',
        labels: {
          fa: 'نودان',
          en: 'Nowdan',
        },
      },
      {
        id: '681',
        labels: {
          fa: 'نورآباد',
          en: 'Nourabad',
        },
      },
      {
        id: '682',
        labels: {
          fa: 'نی ریز',
          en: 'Neyriz',
        },
      },
      {
        id: '683',
        labels: {
          fa: 'وراوی',
          en: 'Varavi',
        },
      },
    ],
  },
  {
    id: 'qazvin',
    labels: {
      fa: 'قزوین',
      en: 'Qazvin',
    },
    cities: [
      {
        id: '684',
        labels: {
          fa: 'ارداق',
          en: 'Ardaq',
        },
      },
      {
        id: '685',
        labels: {
          fa: 'اسفرورین',
          en: 'Asfarvarin',
        },
      },
      {
        id: '686',
        labels: {
          fa: 'اقبالیه',
          en: 'Eqbaliyeh',
        },
      },
      {
        id: '687',
        labels: {
          fa: 'الوند',
          en: 'Alvand',
        },
      },
      {
        id: '688',
        labels: {
          fa: 'آبگرم',
          en: 'Abgarm',
        },
      },
      {
        id: '689',
        labels: {
          fa: 'آبیک',
          en: 'Abyek',
        },
      },
      {
        id: '690',
        labels: {
          fa: 'آوج',
          en: 'Avaj',
        },
      },
      {
        id: '691',
        labels: {
          fa: 'بوئین زهرا',
          en: 'Buin Zahra',
        },
      },
      {
        id: '692',
        labels: {
          fa: 'بیدستان',
          en: 'Bidestan',
        },
      },
      {
        id: '693',
        labels: {
          fa: 'تاکستان',
          en: 'Takestan',
        },
      },
      {
        id: '694',
        labels: {
          fa: 'خاکعلی',
          en: "Khak-e 'Ali",
        },
      },
      {
        id: '695',
        labels: {
          fa: 'خرمدشت',
          en: 'Khorramdasht',
        },
      },
      {
        id: '696',
        labels: {
          fa: 'دانسفهان',
          en: 'Danesfahan',
        },
      },
      {
        id: '697',
        labels: {
          fa: 'رازمیان',
          en: 'Razmian',
        },
      },
      {
        id: '698',
        labels: {
          fa: 'سگزآباد',
          en: 'Saggez Abad',
        },
      },
      {
        id: '699',
        labels: {
          fa: 'سیردان',
          en: 'Sirdan',
        },
      },
      {
        id: '700',
        labels: {
          fa: 'شال',
          en: 'Shal',
        },
      },
      {
        id: '701',
        labels: {
          fa: 'شریفیه',
          en: 'Sharifabad',
        },
      },
      {
        id: '702',
        labels: {
          fa: 'ضیاآباد',
          en: "Zia'abad",
        },
      },
      {
        id: '703',
        labels: {
          fa: 'قزوین',
          en: 'Qazvin',
        },
      },
      {
        id: '704',
        labels: {
          fa: 'کوهین',
          en: 'Kuhin',
        },
      },
      {
        id: '705',
        labels: {
          fa: 'محمدیه',
          en: 'Shahrak-e Mohammadiyeh',
        },
      },
      {
        id: '706',
        labels: {
          fa: 'محمودآباد نمونه',
          en: 'Mahmudabad Nemuneh',
        },
      },
      {
        id: '707',
        labels: {
          fa: 'معلم کلایه',
          en: 'Moallem Kalayeh',
        },
      },
      {
        id: '708',
        labels: {
          fa: 'نرجه',
          en: 'Narjeh',
        },
      },
    ],
  },
  {
    id: 'qom',
    labels: {
      fa: 'قم',
      en: 'Qom',
    },
    cities: [
      {
        id: '709',
        labels: {
          fa: 'جعفریه',
          en: 'Jafariyeh',
        },
      },
      {
        id: '710',
        labels: {
          fa: 'دستجرد',
          en: 'Dastjerd',
        },
      },
      {
        id: '711',
        labels: {
          fa: 'سلفچگان',
          en: 'Salafchegan',
        },
      },
      {
        id: '712',
        labels: {
          fa: 'قم',
          en: 'Qom',
        },
      },
      {
        id: '713',
        labels: {
          fa: 'قنوات',
          en: 'Qanavat',
        },
      },
      {
        id: '714',
        labels: {
          fa: 'کهک',
          en: 'Kahak',
        },
      },
    ],
  },
  {
    id: 'kurdistan',
    labels: {
      fa: 'کردستان',
      en: 'Kurdistan',
    },
    cities: [
      {
        id: '715',
        labels: {
          fa: 'آرمرده',
          en: 'Armardeh',
        },
      },
      {
        id: '716',
        labels: {
          fa: 'بابارشانی',
          en: 'Baba Rashani',
        },
      },
      {
        id: '717',
        labels: {
          fa: 'بانه',
          en: 'Baneh',
        },
      },
      {
        id: '718',
        labels: {
          fa: 'بلبان آباد',
          en: 'Bolbanabad',
        },
      },
      {
        id: '719',
        labels: {
          fa: 'بوئین سفلی',
          en: "Bu'in-e Sofla",
        },
      },
      {
        id: '720',
        labels: {
          fa: 'بیجار',
          en: 'Bijar',
        },
      },
      {
        id: '721',
        labels: {
          fa: 'چناره',
          en: 'Chenareh',
        },
      },
      {
        id: '722',
        labels: {
          fa: 'دزج',
          en: 'Dezaj',
        },
      },
      {
        id: '723',
        labels: {
          fa: 'دلبران',
          en: 'Delbaran',
        },
      },
      {
        id: '724',
        labels: {
          fa: 'دهگلان',
          en: 'Dehgolan',
        },
      },
      {
        id: '725',
        labels: {
          fa: 'دیواندره',
          en: 'Divandarreh',
        },
      },
      {
        id: '726',
        labels: {
          fa: 'زرینه',
          en: 'Zarrineh',
        },
      },
      {
        id: '727',
        labels: {
          fa: 'سروآباد',
          en: 'Sarvabad',
        },
      },
      {
        id: '728',
        labels: {
          fa: 'سریش آباد',
          en: 'Serishabad',
        },
      },
      {
        id: '729',
        labels: {
          fa: 'سقز',
          en: 'Saqqez',
        },
      },
      {
        id: '730',
        labels: {
          fa: 'سنندج',
          en: 'Sanandaj',
        },
      },
      {
        id: '731',
        labels: {
          fa: 'شویشه',
          en: 'Shuyesheh',
        },
      },
      {
        id: '732',
        labels: {
          fa: 'صاحب',
          en: 'Saheb',
        },
      },
      {
        id: '733',
        labels: {
          fa: 'قروه',
          en: 'Qorveh',
        },
      },
      {
        id: '734',
        labels: {
          fa: 'کامیاران',
          en: 'Kamyaran',
        },
      },
      {
        id: '735',
        labels: {
          fa: 'کانی دینار',
          en: 'Dinar',
        },
      },
      {
        id: '736',
        labels: {
          fa: 'کانی سور',
          en: 'Kani Sur',
        },
      },
      {
        id: '737',
        labels: {
          fa: 'مریوان',
          en: 'Marivan',
        },
      },
      {
        id: '738',
        labels: {
          fa: 'موچش',
          en: 'Muchesh',
        },
      },
      {
        id: '739',
        labels: {
          fa: 'یاسوکند',
          en: 'Hasanabad-e Yasukand',
        },
      },
    ],
  },
  {
    id: 'kerman',
    labels: {
      fa: 'کرمان',
      en: 'Kerman',
    },
    cities: [
      {
        id: '740',
        labels: {
          fa: 'اختیارآباد',
          en: 'Ekhtiarabad',
        },
      },
      {
        id: '741',
        labels: {
          fa: 'ارزوئیه',
          en: 'Arzuiyeh',
        },
      },
      {
        id: '742',
        labels: {
          fa: 'امین شهر',
          en: 'Aminshahr',
        },
      },
      {
        id: '743',
        labels: {
          fa: 'انار',
          en: 'Anar',
        },
      },
      {
        id: '744',
        labels: {
          fa: 'اندوهجرد',
          en: 'Anduhjerd',
        },
      },
      {
        id: '745',
        labels: {
          fa: 'باغین',
          en: 'Baghin',
        },
      },
      {
        id: '746',
        labels: {
          fa: 'بافت',
          en: 'Baft',
        },
      },
      {
        id: '747',
        labels: {
          fa: 'بردسیر',
          en: 'Bardsir',
        },
      },
      {
        id: '748',
        labels: {
          fa: 'بروات',
          en: 'Baravat',
        },
      },
      {
        id: '749',
        labels: {
          fa: 'بزنجان',
          en: 'Bezenjan',
        },
      },
      {
        id: '750',
        labels: {
          fa: 'بم',
          en: 'Bam',
        },
      },
      {
        id: '751',
        labels: {
          fa: 'بهرمان',
          en: 'Bahreman',
        },
      },
      {
        id: '752',
        labels: {
          fa: 'پاریز',
          en: 'Pariz',
        },
      },
      {
        id: '753',
        labels: {
          fa: 'جبالبارز',
          en: 'Jebalbarez',
        },
      },
      {
        id: '754',
        labels: {
          fa: 'جوپار',
          en: 'Shahid Jupar',
        },
      },
      {
        id: '755',
        labels: {
          fa: 'جوزم',
          en: 'Javazm',
        },
      },
      {
        id: '756',
        labels: {
          fa: 'جیرفت',
          en: 'Jiroft',
        },
      },
      {
        id: '757',
        labels: {
          fa: 'چترود',
          en: 'Chatrud',
        },
      },
      {
        id: '758',
        labels: {
          fa: 'خاتون آباد',
          en: 'Khatunabad',
        },
      },
      {
        id: '759',
        labels: {
          fa: 'خانوک',
          en: 'Khanuk',
        },
      },
      {
        id: '760',
        labels: {
          fa: 'خورسند',
          en: 'Khorsand',
        },
      },
      {
        id: '761',
        labels: {
          fa: 'درب بهشت',
          en: 'Darb-e Behesht',
        },
      },
      {
        id: '762',
        labels: {
          fa: 'دهج',
          en: 'Dehaj',
        },
      },
      {
        id: '763',
        labels: {
          fa: 'رابر',
          en: 'Rabor',
        },
      },
      {
        id: '764',
        labels: {
          fa: 'راور',
          en: 'Ravar',
        },
      },
      {
        id: '765',
        labels: {
          fa: 'راین',
          en: 'Rayen',
        },
      },
      {
        id: '766',
        labels: {
          fa: 'رفسنجان',
          en: 'Rafsanjan',
        },
      },
      {
        id: '767',
        labels: {
          fa: 'رودبار',
          en: 'Rudbar',
        },
      },
      {
        id: '768',
        labels: {
          fa: 'ریحان شهر',
          en: 'Reyhan Shahr',
        },
      },
      {
        id: '769',
        labels: {
          fa: 'زرند',
          en: 'Zarand',
        },
      },
      {
        id: '770',
        labels: {
          fa: 'زنگی آباد',
          en: 'Zangiabad',
        },
      },
      {
        id: '771',
        labels: {
          fa: 'زیدآباد',
          en: 'Zeydabad',
        },
      },
      {
        id: '772',
        labels: {
          fa: 'سیرجان',
          en: 'Sirjan',
        },
      },
      {
        id: '773',
        labels: {
          fa: 'شهداد',
          en: 'Shahdad',
        },
      },
      {
        id: '774',
        labels: {
          fa: 'شهربابک',
          en: 'Shahr-e Babak',
        },
      },
      {
        id: '775',
        labels: {
          fa: 'صفائیه',
          en: "Safa'iyeh",
        },
      },
      {
        id: '776',
        labels: {
          fa: 'عنبرآباد',
          en: 'Anbarabad',
        },
      },
      {
        id: '777',
        labels: {
          fa: 'فاریاب',
          en: 'Faryab',
        },
      },
      {
        id: '778',
        labels: {
          fa: 'فهرج',
          en: 'Fahraj',
        },
      },
      {
        id: '779',
        labels: {
          fa: 'قلعه گنج',
          en: 'Qaleh Ganj',
        },
      },
      {
        id: '780',
        labels: {
          fa: 'کاظم آباد',
          en: 'Kazemabad',
        },
      },
      {
        id: '781',
        labels: {
          fa: 'کرمان',
          en: 'Kerman',
        },
      },
      {
        id: '782',
        labels: {
          fa: 'کشکوئیه',
          en: "Kashku'iyeh",
        },
      },
      {
        id: '783',
        labels: {
          fa: 'کهنوج',
          en: 'Kahnuj',
        },
      },
      {
        id: '784',
        labels: {
          fa: 'کوهبنان',
          en: 'Kuhbanan',
        },
      },
      {
        id: '785',
        labels: {
          fa: 'کیانشهر',
          en: "Shahrak-e Qa'em Shahr",
        },
      },
      {
        id: '786',
        labels: {
          fa: 'گلباف',
          en: 'Golbaf',
        },
      },
      {
        id: '787',
        labels: {
          fa: 'گلزار',
          en: 'Golzar',
        },
      },
      {
        id: '788',
        labels: {
          fa: 'لاله زار',
          en: 'Lalehzar',
        },
      },
      {
        id: '789',
        labels: {
          fa: 'ماهان',
          en: 'Mahan',
        },
      },
      {
        id: '790',
        labels: {
          fa: 'محمدآباد',
          en: 'Mohammadabad',
        },
      },
      {
        id: '791',
        labels: {
          fa: 'محی آباد',
          en: 'Mohiabad',
        },
      },
      {
        id: '792',
        labels: {
          fa: 'مردهک',
          en: 'Mardehek',
        },
      },
      {
        id: '793',
        labels: {
          fa: 'مس سرچشمه',
          en: 'Mes-e Sarcheshmeh',
        },
      },
      {
        id: '794',
        labels: {
          fa: 'منوجان',
          en: 'Manujan',
        },
      },
      {
        id: '795',
        labels: {
          fa: 'نجف شهر',
          en: 'Najaf Shahr',
        },
      },
      {
        id: '796',
        labels: {
          fa: 'نرماشیر',
          en: 'Narmashir',
        },
      },
      {
        id: '797',
        labels: {
          fa: 'نظام شهر',
          en: 'Nezamabad',
        },
      },
      {
        id: '798',
        labels: {
          fa: 'نگار',
          en: 'Negar',
        },
      },
      {
        id: '799',
        labels: {
          fa: 'نودژ',
          en: 'Now Dezh-e Markazi',
        },
      },
      {
        id: '800',
        labels: {
          fa: 'هجدک',
          en: 'Hojdak',
        },
      },
      {
        id: '801',
        labels: {
          fa: 'یزدان شهر',
          en: 'Yazdan Shahr',
        },
      },
    ],
  },
  {
    id: 'kermanshah',
    labels: {
      fa: 'کرمانشاه',
      en: 'Kermanshah',
    },
    cities: [
      {
        id: '802',
        labels: {
          fa: 'ازگله',
          en: 'Ozgoleh',
        },
      },
      {
        id: '803',
        labels: {
          fa: 'اسلام آباد غرب',
          en: 'Eslamabad-e Gharb',
        },
      },
      {
        id: '804',
        labels: {
          fa: 'باینگان',
          en: 'Bayangan',
        },
      },
      {
        id: '805',
        labels: {
          fa: 'بیستون',
          en: 'Bisotun',
        },
      },
      {
        id: '806',
        labels: {
          fa: 'پاوه',
          en: 'Paveh',
        },
      },
      {
        id: '807',
        labels: {
          fa: 'تازه آباد',
          en: 'Tazehabad',
        },
      },
      {
        id: '808',
        labels: {
          fa: 'جوان رود',
          en: 'Javanrud',
        },
      },
      {
        id: '809',
        labels: {
          fa: 'حمیل',
          en: 'Homeyl',
        },
      },
      {
        id: '810',
        labels: {
          fa: 'ماهیدشت',
          en: 'Mahidasht',
        },
      },
      {
        id: '811',
        labels: {
          fa: 'روانسر',
          en: 'Ravansar',
        },
      },
      {
        id: '812',
        labels: {
          fa: 'سرپل ذهاب',
          en: 'Sarpol-e Zahab',
        },
      },
      {
        id: '813',
        labels: {
          fa: 'سرمست',
          en: 'Sarmast',
        },
      },
      {
        id: '814',
        labels: {
          fa: 'سطر',
          en: 'Satar',
        },
      },
      {
        id: '815',
        labels: {
          fa: 'سنقر',
          en: 'Sonqor',
        },
      },
      {
        id: '816',
        labels: {
          fa: 'سومار',
          en: 'Sumar',
        },
      },
      {
        id: '817',
        labels: {
          fa: 'شاهو',
          en: 'Shahu',
        },
      },
      {
        id: '818',
        labels: {
          fa: 'صحنه',
          en: 'Sahneh',
        },
      },
      {
        id: '819',
        labels: {
          fa: 'قصرشیرین',
          en: 'Qasr-e Shirin',
        },
      },
      {
        id: '820',
        labels: {
          fa: 'کرمانشاه',
          en: 'Kermanshah',
        },
      },
      {
        id: '821',
        labels: {
          fa: 'کرندغرب',
          en: 'Kerend-e Gharb',
        },
      },
      {
        id: '822',
        labels: {
          fa: 'کنگاور',
          en: 'Kangavar',
        },
      },
      {
        id: '823',
        labels: {
          fa: 'کوزران',
          en: 'Kuzaran',
        },
      },
      {
        id: '824',
        labels: {
          fa: 'گهواره',
          en: 'Gahvareh',
        },
      },
      {
        id: '825',
        labels: {
          fa: 'گیلانغرب',
          en: 'Gilan-e Gharb',
        },
      },
      {
        id: '826',
        labels: {
          fa: 'میان راهان',
          en: 'Mian Rahan',
        },
      },
      {
        id: '827',
        labels: {
          fa: 'نودشه',
          en: 'Nowdeshah',
        },
      },
      {
        id: '828',
        labels: {
          fa: 'نوسود',
          en: 'Nowsud',
        },
      },
      {
        id: '829',
        labels: {
          fa: 'هرسین',
          en: 'Harsin',
        },
      },
      {
        id: '830',
        labels: {
          fa: 'هلشی',
          en: 'Halashi',
        },
      },
    ],
  },
  {
    id: 'kohgiluyeh-and-boyer-ahmad',
    labels: {
      fa: 'کهگیلویه و بویراحمد',
      en: 'Kohgiluyeh and Boyer-Ahmad',
    },
    cities: [
      {
        id: '831',
        labels: {
          fa: 'باشت',
          en: 'Basht',
        },
      },
      {
        id: '832',
        labels: {
          fa: 'پاتاوه',
          en: 'Pataveh',
        },
      },
      {
        id: '833',
        labels: {
          fa: 'چرام',
          en: 'Charam',
        },
      },
      {
        id: '834',
        labels: {
          fa: 'چیتاب',
          en: 'Chitab',
        },
      },
      {
        id: '835',
        labels: {
          fa: 'دهدشت',
          en: 'Dehdasht',
        },
      },
      {
        id: '836',
        labels: {
          fa: 'دوگنبدان',
          en: 'Dogonbadan',
        },
      },
      {
        id: '837',
        labels: {
          fa: 'دیشموک',
          en: 'Dishmuk',
        },
      },
      {
        id: '838',
        labels: {
          fa: 'سوق',
          en: 'Suq',
        },
      },
      {
        id: '839',
        labels: {
          fa: 'سی سخت',
          en: 'Sisakht',
        },
      },
      {
        id: '840',
        labels: {
          fa: 'قلعه رئیسی',
          en: "Qal'eh Ris",
        },
      },
      {
        id: '841',
        labels: {
          fa: 'گراب سفلی',
          en: 'Garab-e Bala',
        },
      },
      {
        id: '842',
        labels: {
          fa: 'لنده',
          en: 'Landeh',
        },
      },
      {
        id: '843',
        labels: {
          fa: 'لیکک',
          en: 'Likak',
        },
      },
      {
        id: '844',
        labels: {
          fa: 'مادوان',
          en: "Madavan-e 'Olya",
        },
      },
      {
        id: '845',
        labels: {
          fa: 'مارگون',
          en: 'Margown',
        },
      },
      {
        id: '846',
        labels: {
          fa: 'یاسوج',
          en: 'Yasuj',
        },
      },
    ],
  },
  {
    id: 'golestan',
    labels: {
      fa: 'گلستان',
      en: 'Golestan',
    },
    cities: [
      {
        id: '847',
        labels: {
          fa: 'انبارآلوم',
          en: 'Anbar Alum',
        },
      },
      {
        id: '848',
        labels: {
          fa: 'اینچه برون',
          en: 'Incheh Borun',
        },
      },
      {
        id: '849',
        labels: {
          fa: 'آزادشهر',
          en: 'Azadshahr',
        },
      },
      {
        id: '850',
        labels: {
          fa: 'آق قلا',
          en: 'Aqqala',
        },
      },
      {
        id: '851',
        labels: {
          fa: 'بندرترکمن',
          en: 'Bandar-e Torkaman',
        },
      },
      {
        id: '852',
        labels: {
          fa: 'بندرگز',
          en: 'Bandar-e Gaz',
        },
      },
      {
        id: '853',
        labels: {
          fa: 'جلین',
          en: 'Jelin',
        },
      },
      {
        id: '854',
        labels: {
          fa: 'خان ببین',
          en: 'Khan Bebin',
        },
      },
      {
        id: '855',
        labels: {
          fa: 'دلند',
          en: 'Daland',
        },
      },
      {
        id: '856',
        labels: {
          fa: 'رامیان',
          en: 'Ramian',
        },
      },
      {
        id: '857',
        labels: {
          fa: 'سرخنکلاته',
          en: 'Sarkhonkalateh',
        },
      },
      {
        id: '858',
        labels: {
          fa: 'سیمین شهر',
          en: 'Simin Shahr',
        },
      },
      {
        id: '859',
        labels: {
          fa: 'علی آباد کتول',
          en: 'Aliabad Katoul',
        },
      },
      {
        id: '860',
        labels: {
          fa: 'فاضل آباد',
          en: 'Fazelabad',
        },
      },
      {
        id: '861',
        labels: {
          fa: 'کردکوی',
          en: 'Kordkuy',
        },
      },
      {
        id: '862',
        labels: {
          fa: 'کلاله',
          en: 'Kalaleh',
        },
      },
      {
        id: '863',
        labels: {
          fa: 'گالیکش',
          en: 'Galikesh',
        },
      },
      {
        id: '864',
        labels: {
          fa: 'گرگان',
          en: 'Gorgan',
        },
      },
      {
        id: '865',
        labels: {
          fa: 'گمیش تپه',
          en: 'Gomishan',
        },
      },
      {
        id: '866',
        labels: {
          fa: 'گنبدکاووس',
          en: 'Gonbad-e Qabus',
        },
      },
      {
        id: '867',
        labels: {
          fa: 'مراوه',
          en: 'Maraveh',
        },
      },
      {
        id: '868',
        labels: {
          fa: 'مینودشت',
          en: 'Minudasht',
        },
      },
      {
        id: '869',
        labels: {
          fa: 'نگین شهر',
          en: 'Neginshahr',
        },
      },
      {
        id: '870',
        labels: {
          fa: 'نوده خاندوز',
          en: 'Now Deh-e Khanduz',
        },
      },
      {
        id: '871',
        labels: {
          fa: 'نوکنده',
          en: 'Now Kandeh',
        },
      },
    ],
  },
  {
    id: 'lorestan',
    labels: {
      fa: 'لرستان',
      en: 'Lorestan',
    },
    cities: [
      {
        id: '872',
        labels: {
          fa: 'ازنا',
          en: 'Azna',
        },
      },
      {
        id: '873',
        labels: {
          fa: 'اشترینان',
          en: 'Oshtorinan',
        },
      },
      {
        id: '874',
        labels: {
          fa: 'الشتر',
          en: 'Alashtar',
        },
      },
      {
        id: '875',
        labels: {
          fa: 'الیگودرز',
          en: 'Aligudarz',
        },
      },
      {
        id: '876',
        labels: {
          fa: 'بروجرد',
          en: 'Borujerd',
        },
      },
      {
        id: '877',
        labels: {
          fa: 'پلدختر',
          en: 'Pol-e Dokhtar',
        },
      },
      {
        id: '878',
        labels: {
          fa: 'چالانچولان',
          en: 'Chalan Chulan',
        },
      },
      {
        id: '879',
        labels: {
          fa: 'چغلوندی',
          en: 'Chaghalvandi',
        },
      },
      {
        id: '880',
        labels: {
          fa: 'چقابل',
          en: 'Chaqabol',
        },
      },
      {
        id: '881',
        labels: {
          fa: 'خرم آباد',
          en: 'Khorramabad',
        },
      },
      {
        id: '882',
        labels: {
          fa: 'درب گنبد',
          en: 'Shahzadeh Mohammad',
        },
      },
      {
        id: '883',
        labels: {
          fa: 'دورود',
          en: 'Dorud',
        },
      },
      {
        id: '884',
        labels: {
          fa: 'زاغه',
          en: 'Zagheh',
        },
      },
      {
        id: '885',
        labels: {
          fa: 'سپیددشت',
          en: 'Sepid Dasht',
        },
      },
      {
        id: '886',
        labels: {
          fa: 'سراب دوره',
          en: 'Sarab-e Dureh',
        },
      },
      {
        id: '887',
        labels: {
          fa: 'فیروزآباد',
          en: 'Firuzabad',
        },
      },
      {
        id: '888',
        labels: {
          fa: 'کونانی',
          en: 'Kuhnani',
        },
      },
      {
        id: '889',
        labels: {
          fa: 'کوهدشت',
          en: 'Kuhdasht',
        },
      },
      {
        id: '890',
        labels: {
          fa: 'گراب',
          en: 'Garab',
        },
      },
      {
        id: '891',
        labels: {
          fa: 'معمولان',
          en: "Ma'mulan",
        },
      },
      {
        id: '892',
        labels: {
          fa: 'مومن آباد',
          en: "Mow'menabad",
        },
      },
      {
        id: '893',
        labels: {
          fa: 'نورآباد',
          en: 'Nourabad',
        },
      },
      {
        id: '894',
        labels: {
          fa: 'ویسیان',
          en: 'Veysian',
        },
      },
    ],
  },
  {
    id: 'gilan',
    labels: {
      fa: 'گیلان',
      en: 'Gilan',
    },
    cities: [
      {
        id: '895',
        labels: {
          fa: 'احمدسرگوراب',
          en: 'Ahmad Sar Gurab',
        },
      },
      {
        id: '896',
        labels: {
          fa: 'اسالم',
          en: 'Asalem',
        },
      },
      {
        id: '897',
        labels: {
          fa: 'اطاقور',
          en: 'Otaqvar',
        },
      },
      {
        id: '898',
        labels: {
          fa: 'املش',
          en: 'Amlash',
        },
      },
      {
        id: '899',
        labels: {
          fa: 'آستارا',
          en: 'Astara',
        },
      },
      {
        id: '900',
        labels: {
          fa: 'آستانه اشرفیه',
          en: 'Astaneh-ye Ashrafiyeh',
        },
      },
      {
        id: '901',
        labels: {
          fa: 'بازار جمعه',
          en: "Bazar Jom'eh-ye Shanderman",
        },
      },
      {
        id: '902',
        labels: {
          fa: 'بره سر',
          en: 'Bareh Sar',
        },
      },
      {
        id: '903',
        labels: {
          fa: 'بندرانزلی',
          en: 'Bandar-e Anzali',
        },
      },
      {
        id: '906',
        labels: {
          fa: 'پره سر',
          en: 'Pareh Sar',
        },
      },
      {
        id: '907',
        labels: {
          fa: 'تالش',
          en: 'Talesh',
        },
      },
      {
        id: '908',
        labels: {
          fa: 'توتکابن',
          en: 'Tutkabon',
        },
      },
      {
        id: '909',
        labels: {
          fa: 'جیرنده',
          en: 'Jirandeh',
        },
      },
      {
        id: '910',
        labels: {
          fa: 'چابکسر',
          en: 'Chaboksar',
        },
      },
      {
        id: '911',
        labels: {
          fa: 'چاف و چمخاله',
          en: "Pa'in Chaf",
        },
      },
      {
        id: '912',
        labels: {
          fa: 'چوبر',
          en: 'Chubar',
        },
      },
      {
        id: '913',
        labels: {
          fa: 'حویق',
          en: 'Haviq',
        },
      },
      {
        id: '914',
        labels: {
          fa: 'خشکبیجار',
          en: 'Khoshk Bijar',
        },
      },
      {
        id: '915',
        labels: {
          fa: 'خمام',
          en: 'Khomam',
        },
      },
      {
        id: '916',
        labels: {
          fa: 'دیلمان',
          en: 'Deylaman',
        },
      },
      {
        id: '917',
        labels: {
          fa: 'رانکوه',
          en: 'Rankuh',
        },
      },
      {
        id: '918',
        labels: {
          fa: 'رحیم آباد',
          en: 'Rahimabad',
        },
      },
      {
        id: '919',
        labels: {
          fa: 'رستم آباد',
          en: 'Rostamabad',
        },
      },
      {
        id: '920',
        labels: {
          fa: 'رشت',
          en: 'Rasht',
        },
      },
      {
        id: '921',
        labels: {
          fa: 'رضوانشهر',
          en: 'Rezvanshahr',
        },
      },
      {
        id: '922',
        labels: {
          fa: 'رودبار',
          en: 'Rudbar',
        },
      },
      {
        id: '923',
        labels: {
          fa: 'رودبنه',
          en: 'Rudboneh',
        },
      },
      {
        id: '924',
        labels: {
          fa: 'رودسر',
          en: 'Rudsar',
        },
      },
      {
        id: '925',
        labels: {
          fa: 'سنگر',
          en: 'Sangar',
        },
      },
      {
        id: '926',
        labels: {
          fa: 'سیاهکل',
          en: 'Siahkal',
        },
      },
      {
        id: '927',
        labels: {
          fa: 'شفت',
          en: 'Shaft',
        },
      },
      {
        id: '928',
        labels: {
          fa: 'شلمان',
          en: 'Shalman',
        },
      },
      {
        id: '929',
        labels: {
          fa: 'صومعه سرا',
          en: 'Sowme-eh Sara',
        },
      },
      {
        id: '930',
        labels: {
          fa: 'فومن',
          en: 'Fuman',
        },
      },
      {
        id: '931',
        labels: {
          fa: 'کلاچای',
          en: 'Kala Chay',
        },
      },
      {
        id: '932',
        labels: {
          fa: 'کوچصفهان',
          en: 'Kuchesfahan',
        },
      },
      {
        id: '933',
        labels: {
          fa: 'کومله',
          en: 'Kumeleh',
        },
      },
      {
        id: '934',
        labels: {
          fa: 'کیاشهر',
          en: 'Kiashahr',
        },
      },
      {
        id: '935',
        labels: {
          fa: 'گوراب زرمیخ',
          en: 'Gurab Zarmikh',
        },
      },
      {
        id: '936',
        labels: {
          fa: 'لاهیجان',
          en: 'Lahijan',
        },
      },
      {
        id: '937',
        labels: {
          fa: 'لشت نشا',
          en: 'Lasht-e Nesha',
        },
      },
      {
        id: '938',
        labels: {
          fa: 'لنگرود',
          en: 'Langarud',
        },
      },
      {
        id: '939',
        labels: {
          fa: 'لوشان',
          en: 'Lowshan',
        },
      },
      {
        id: '940',
        labels: {
          fa: 'لولمان',
          en: 'Lulaman',
        },
      },
      {
        id: '941',
        labels: {
          fa: 'لوندویل',
          en: 'Lavandevil',
        },
      },
      {
        id: '942',
        labels: {
          fa: 'لیسار',
          en: 'Lisar',
        },
      },
      {
        id: '943',
        labels: {
          fa: 'ماسال',
          en: 'Masal',
        },
      },
      {
        id: '944',
        labels: {
          fa: 'ماسوله',
          en: 'Masouleh',
        },
      },
      {
        id: '945',
        labels: {
          fa: 'مرجقل',
          en: 'Marjaghal',
        },
      },
      {
        id: '946',
        labels: {
          fa: 'منجیل',
          en: 'Manjil',
        },
      },
      {
        id: '947',
        labels: {
          fa: 'واجارگاه',
          en: 'Vejargah',
        },
      },
      {
        id: '1121',
        labels: {
          fa: 'زیباکنار',
          en: 'Ziba Kenar',
        },
      },
      {
        id: '1159',
        labels: {
          fa: 'پیربازار',
          en: 'Pir Bazar',
        },
      },
    ],
  },
  {
    id: 'mazandaran',
    labels: {
      fa: 'مازندران',
      en: 'Mazandaran',
    },
    cities: [
      {
        id: '948',
        labels: {
          fa: 'امیرکلا',
          en: 'Amir Kola',
        },
      },
      {
        id: '949',
        labels: {
          fa: 'ایزدشهر',
          en: 'Izadshahr',
        },
      },
      {
        id: '950',
        labels: {
          fa: 'آلاشت',
          en: 'Alasht',
        },
      },
      {
        id: '951',
        labels: {
          fa: 'آمل',
          en: 'Amol',
        },
      },
      {
        id: '952',
        labels: {
          fa: 'بابل',
          en: 'Babol',
        },
      },
      {
        id: '953',
        labels: {
          fa: 'بابلسر',
          en: 'Babolsar',
        },
      },
      {
        id: '954',
        labels: {
          fa: 'بلده',
          en: 'Baladeh',
        },
      },
      {
        id: '955',
        labels: {
          fa: 'بهشهر',
          en: 'Behshahr',
        },
      },
      {
        id: '956',
        labels: {
          fa: 'بهنمیر',
          en: 'Bahnemir',
        },
      },
      {
        id: '957',
        labels: {
          fa: 'پل سفید',
          en: 'Pol-e Sefid',
        },
      },
      {
        id: '958',
        labels: {
          fa: 'تنکابن',
          en: 'Tonekabon',
        },
      },
      {
        id: '959',
        labels: {
          fa: 'جویبار',
          en: 'Juybar',
        },
      },
      {
        id: '960',
        labels: {
          fa: 'چالوس',
          en: 'Chalus',
        },
      },
      {
        id: '961',
        labels: {
          fa: 'چمستان',
          en: 'Chamestan',
        },
      },
      {
        id: '962',
        labels: {
          fa: 'خرم آباد',
          en: 'Khorramabad',
        },
      },
      {
        id: '963',
        labels: {
          fa: 'خلیل شهر',
          en: 'Khalil Shahr',
        },
      },
      {
        id: '964',
        labels: {
          fa: 'خوش رودپی',
          en: 'Khush Rudpey',
        },
      },
      {
        id: '965',
        labels: {
          fa: 'دابودشت',
          en: 'Dabudasht',
        },
      },
      {
        id: '966',
        labels: {
          fa: 'رامسر',
          en: 'Ramsar',
        },
      },
      {
        id: '967',
        labels: {
          fa: 'رستمکلا',
          en: 'Rostam Kola',
        },
      },
      {
        id: '968',
        labels: {
          fa: 'رویان',
          en: 'Ruyan',
        },
      },
      {
        id: '969',
        labels: {
          fa: 'رینه',
          en: 'Rineh',
        },
      },
      {
        id: '970',
        labels: {
          fa: 'زرگرمحله',
          en: 'Zargar Mahalleh',
        },
      },
      {
        id: '971',
        labels: {
          fa: 'زیرآب',
          en: 'Zir Ab',
        },
      },
      {
        id: '972',
        labels: {
          fa: 'ساری',
          en: 'Sari',
        },
      },
      {
        id: '973',
        labels: {
          fa: 'سرخرود',
          en: 'Sorkhrud',
        },
      },
      {
        id: '974',
        labels: {
          fa: 'سلمان شهر',
          en: 'Salmanshahr',
        },
      },
      {
        id: '975',
        labels: {
          fa: 'سورک',
          en: 'Surak',
        },
      },
      {
        id: '976',
        labels: {
          fa: 'شیرگاه',
          en: 'Shir Gah',
        },
      },
      {
        id: '977',
        labels: {
          fa: 'شیرود',
          en: 'Shirud',
        },
      },
      {
        id: '978',
        labels: {
          fa: 'عباس آباد',
          en: 'Abbasabad',
        },
      },
      {
        id: '979',
        labels: {
          fa: 'فریدونکنار',
          en: 'Fereydunkenar',
        },
      },
      {
        id: '980',
        labels: {
          fa: 'فریم',
          en: 'Farim',
        },
      },
      {
        id: '981',
        labels: {
          fa: 'قائم شهر',
          en: 'Qaem Shahr',
        },
      },
      {
        id: '982',
        labels: {
          fa: 'کتالم',
          en: 'Ketalem',
        },
      },
      {
        id: '983',
        labels: {
          fa: 'کلارآباد',
          en: 'Kelarabad',
        },
      },
      {
        id: '984',
        labels: {
          fa: 'کلاردشت',
          en: 'Kelardasht',
        },
      },
      {
        id: '985',
        labels: {
          fa: 'کله بست',
          en: 'Kalleh Bast',
        },
      },
      {
        id: '986',
        labels: {
          fa: 'کوهی خیل',
          en: 'Kuhi Kheyl',
        },
      },
      {
        id: '987',
        labels: {
          fa: 'کیاسر',
          en: 'Kiasar-e Bala',
        },
      },
      {
        id: '988',
        labels: {
          fa: 'کیاکلا',
          en: 'Kia Kola',
        },
      },
      {
        id: '989',
        labels: {
          fa: 'گتاب',
          en: 'Gatab',
        },
      },
      {
        id: '990',
        labels: {
          fa: 'گزنک',
          en: 'Gazanak',
        },
      },
      {
        id: '991',
        labels: {
          fa: 'گلوگاه',
          en: 'Galugah',
        },
      },
      {
        id: '992',
        labels: {
          fa: 'محمودآباد',
          en: 'Mahmudabad',
        },
      },
      {
        id: '993',
        labels: {
          fa: 'مرزن آباد',
          en: 'Marzanabad',
        },
      },
      {
        id: '994',
        labels: {
          fa: 'مرزیکلا',
          en: 'Marzi Kola',
        },
      },
      {
        id: '995',
        labels: {
          fa: 'نشتارود',
          en: 'Nashtarud',
        },
      },
      {
        id: '996',
        labels: {
          fa: 'نکا',
          en: 'Neka',
        },
      },
      {
        id: '997',
        labels: {
          fa: 'نور',
          en: 'Nur',
        },
      },
      {
        id: '998',
        labels: {
          fa: 'نوشهر',
          en: 'Nowshahr',
        },
      },
      {
        id: '1119',
        labels: {
          fa: 'سادات شهر',
          en: 'Katalem va Sadat Shahr',
        },
      },
    ],
  },
  {
    id: 'markazi',
    labels: {
      fa: 'مرکزی',
      en: 'Markazi',
    },
    cities: [
      {
        id: '999',
        labels: {
          fa: 'اراک',
          en: 'Arak',
        },
      },
      {
        id: '1000',
        labels: {
          fa: 'آستانه',
          en: 'Astaneh',
        },
      },
      {
        id: '1001',
        labels: {
          fa: 'آشتیان',
          en: 'Ashtian',
        },
      },
      {
        id: '1002',
        labels: {
          fa: 'پرندک',
          en: 'Rahimabad-e Parandak',
        },
      },
      {
        id: '1003',
        labels: {
          fa: 'تفرش',
          en: 'Tafresh',
        },
      },
      {
        id: '1004',
        labels: {
          fa: 'توره',
          en: 'Tureh',
        },
      },
      {
        id: '1005',
        labels: {
          fa: 'جاورسیان',
          en: 'Javerseyan',
        },
      },
      {
        id: '1006',
        labels: {
          fa: 'خشکرود',
          en: 'Khoshkrud',
        },
      },
      {
        id: '1007',
        labels: {
          fa: 'خمین',
          en: 'Khomeyn',
        },
      },
      {
        id: '1008',
        labels: {
          fa: 'خنداب',
          en: 'Khondab',
        },
      },
      {
        id: '1009',
        labels: {
          fa: 'داودآباد',
          en: 'Davudabad',
        },
      },
      {
        id: '1010',
        labels: {
          fa: 'دلیجان',
          en: 'Delijan',
        },
      },
      {
        id: '1011',
        labels: {
          fa: 'رازقان',
          en: 'Razeqan',
        },
      },
      {
        id: '1012',
        labels: {
          fa: 'زاویه',
          en: 'Zaviyeh',
        },
      },
      {
        id: '1013',
        labels: {
          fa: 'ساروق',
          en: 'Saruq',
        },
      },
      {
        id: '1014',
        labels: {
          fa: 'ساوه',
          en: 'Saveh',
        },
      },
      {
        id: '1015',
        labels: {
          fa: 'سنجان',
          en: 'Senjan',
        },
      },
      {
        id: '1016',
        labels: {
          fa: 'شازند',
          en: 'Shazand',
        },
      },
      {
        id: '1017',
        labels: {
          fa: 'غرق آباد',
          en: 'Gharqabad',
        },
      },
      {
        id: '1018',
        labels: {
          fa: 'فرمهین',
          en: 'Farmahin',
        },
      },
      {
        id: '1019',
        labels: {
          fa: 'قورچی باشی',
          en: 'Khorram Dasht',
        },
      },
      {
        id: '1020',
        labels: {
          fa: 'کرهرود',
          en: 'Karahrud',
        },
      },
      {
        id: '1021',
        labels: {
          fa: 'کمیجان',
          en: 'Komijan',
        },
      },
      {
        id: '1022',
        labels: {
          fa: 'مامونیه',
          en: 'Mamuniyeh',
        },
      },
      {
        id: '1023',
        labels: {
          fa: 'محلات',
          en: 'Mahallat',
        },
      },
      {
        id: '1024',
        labels: {
          fa: 'مهاجران',
          en: 'Mahajeran',
        },
      },
      {
        id: '1025',
        labels: {
          fa: 'میلاجرد',
          en: 'Milajerd',
        },
      },
      {
        id: '1026',
        labels: {
          fa: 'نراق',
          en: 'Naraq',
        },
      },
      {
        id: '1027',
        labels: {
          fa: 'نوبران',
          en: 'Nowbaran',
        },
      },
      {
        id: '1028',
        labels: {
          fa: 'نیمور',
          en: 'Nimvar',
        },
      },
      {
        id: '1029',
        labels: {
          fa: 'هندودر',
          en: 'Hendudar',
        },
      },
    ],
  },
  {
    id: 'hormozgan',
    labels: {
      fa: 'هرمزگان',
      en: 'Hormozgan',
    },
    cities: [
      {
        id: '1030',
        labels: {
          fa: 'ابوموسی',
          en: 'Abumusa',
        },
      },
      {
        id: '1031',
        labels: {
          fa: 'بستک',
          en: 'Bastak',
        },
      },
      {
        id: '1032',
        labels: {
          fa: 'بندرجاسک',
          en: 'Bandar Jask',
        },
      },
      {
        id: '1033',
        labels: {
          fa: 'بندرچارک',
          en: 'Bandar-e Charak',
        },
      },
      {
        id: '1034',
        labels: {
          fa: 'بندرخمیر',
          en: 'Bandar Khomir',
        },
      },
      {
        id: '1035',
        labels: {
          fa: 'بندرعباس',
          en: 'Bandar Abbas',
        },
      },
      {
        id: '1036',
        labels: {
          fa: 'بندرلنگه',
          en: 'Bandar Lengeh',
        },
      },
      {
        id: '1037',
        labels: {
          fa: 'بیکا',
          en: 'Bi Kah',
        },
      },
      {
        id: '1038',
        labels: {
          fa: 'پارسیان',
          en: 'Parsian',
        },
      },
      {
        id: '1039',
        labels: {
          fa: 'تخت',
          en: 'Takht',
        },
      },
      {
        id: '1040',
        labels: {
          fa: 'جناح',
          en: 'Jenah',
        },
      },
      {
        id: '1041',
        labels: {
          fa: 'حاجی آباد',
          en: 'Hajjiabad',
        },
      },
      {
        id: '1042',
        labels: {
          fa: 'درگهان',
          en: 'Dargahan',
        },
      },
      {
        id: '1043',
        labels: {
          fa: 'دهبارز',
          en: 'Dehbarez',
        },
      },
      {
        id: '1044',
        labels: {
          fa: 'رویدر',
          en: 'Ruydar',
        },
      },
      {
        id: '1045',
        labels: {
          fa: 'زیارتعلی',
          en: "Ziarat 'Ali",
        },
      },
      {
        id: '1046',
        labels: {
          fa: 'سردشت',
          en: 'Sardasht',
        },
      },
      {
        id: '1047',
        labels: {
          fa: 'سندرک',
          en: 'Senderk',
        },
      },
      {
        id: '1048',
        labels: {
          fa: 'سوزا',
          en: 'Suza',
        },
      },
      {
        id: '1049',
        labels: {
          fa: 'سیریک',
          en: 'Sirik',
        },
      },
      {
        id: '1050',
        labels: {
          fa: 'فارغان',
          en: 'Fareghan',
        },
      },
      {
        id: '1051',
        labels: {
          fa: 'فین',
          en: 'Fin',
        },
      },
      {
        id: '1052',
        labels: {
          fa: 'قشم',
          en: 'Qeshm',
        },
      },
      {
        id: '1053',
        labels: {
          fa: 'قلعه قاضی',
          en: "Qal'eh Qazi",
        },
      },
      {
        id: '1054',
        labels: {
          fa: 'کنگ',
          en: 'Bandar-e Kong',
        },
      },
      {
        id: '1055',
        labels: {
          fa: 'کوشکنار',
          en: 'Kushkonar',
        },
      },
      {
        id: '1056',
        labels: {
          fa: 'کیش',
          en: 'Kish',
        },
      },
      {
        id: '1057',
        labels: {
          fa: 'گوهران',
          en: 'Gowharan',
        },
      },
      {
        id: '1058',
        labels: {
          fa: 'میناب',
          en: 'Minab',
        },
      },
      {
        id: '1059',
        labels: {
          fa: 'هرمز',
          en: 'Hormoz',
        },
      },
      {
        id: '1060',
        labels: {
          fa: 'هشتبندی',
          en: 'Hasht Bandi',
        },
      },
    ],
  },
  {
    id: 'hamadan',
    labels: {
      fa: 'همدان',
      en: 'Hamadan',
    },
    cities: [
      {
        id: '1061',
        labels: {
          fa: 'ازندریان',
          en: 'Azandarian',
        },
      },
      {
        id: '1062',
        labels: {
          fa: 'اسدآباد',
          en: 'Asadabad',
        },
      },
      {
        id: '1063',
        labels: {
          fa: 'برزول',
          en: 'Barzul',
        },
      },
      {
        id: '1064',
        labels: {
          fa: 'بهار',
          en: 'Bahar',
        },
      },
      {
        id: '1065',
        labels: {
          fa: 'تویسرکان',
          en: 'Tuyserkan',
        },
      },
      {
        id: '1066',
        labels: {
          fa: 'جورقان',
          en: 'Juraqan',
        },
      },
      {
        id: '1067',
        labels: {
          fa: 'جوکار',
          en: 'Jowkar',
        },
      },
      {
        id: '1068',
        labels: {
          fa: 'دمق',
          en: 'Damaq',
        },
      },
      {
        id: '1069',
        labels: {
          fa: 'رزن',
          en: 'Razan',
        },
      },
      {
        id: '1070',
        labels: {
          fa: 'زنگنه',
          en: 'Zangeneh',
        },
      },
      {
        id: '1071',
        labels: {
          fa: 'سامن',
          en: 'Samen',
        },
      },
      {
        id: '1072',
        labels: {
          fa: 'سرکان',
          en: 'Sarkan',
        },
      },
      {
        id: '1073',
        labels: {
          fa: 'شیرین سو',
          en: 'Shirin Su',
        },
      },
      {
        id: '1074',
        labels: {
          fa: 'صالح آباد',
          en: 'Salehabad',
        },
      },
      {
        id: '1075',
        labels: {
          fa: 'فامنین',
          en: 'Famenin',
        },
      },
      {
        id: '1076',
        labels: {
          fa: 'فرسفج',
          en: 'Farsafaj',
        },
      },
      {
        id: '1077',
        labels: {
          fa: 'فیروزان',
          en: 'Firuzan',
        },
      },
      {
        id: '1078',
        labels: {
          fa: 'قروه درجزین',
          en: 'Qorveh-e Darjazin',
        },
      },
      {
        id: '1079',
        labels: {
          fa: 'قهاوند',
          en: 'Qahavand',
        },
      },
      {
        id: '1080',
        labels: {
          fa: 'کبودر آهنگ',
          en: 'Kabudarahang',
        },
      },
      {
        id: '1081',
        labels: {
          fa: 'گل تپه',
          en: 'Gol Tappeh',
        },
      },
      {
        id: '1082',
        labels: {
          fa: 'گیان',
          en: 'Kian',
        },
      },
      {
        id: '1083',
        labels: {
          fa: 'لالجین',
          en: 'Lalejin',
        },
      },
      {
        id: '1084',
        labels: {
          fa: 'مریانج',
          en: 'Maryanaj',
        },
      },
      {
        id: '1085',
        labels: {
          fa: 'ملایر',
          en: 'Malayer',
        },
      },
      {
        id: '1086',
        labels: {
          fa: 'نهاوند',
          en: 'Nahavand',
        },
      },
      {
        id: '1087',
        labels: {
          fa: 'همدان',
          en: 'Hamadan',
        },
      },
    ],
  },
  {
    id: 'yazd',
    labels: {
      fa: 'یزد',
      en: 'Yazd',
    },
    cities: [
      {
        id: '1088',
        labels: {
          fa: 'ابرکوه',
          en: 'Abarkuh',
        },
      },
      {
        id: '1089',
        labels: {
          fa: 'احمدآباد',
          en: 'Ahmadabad',
        },
      },
      {
        id: '1090',
        labels: {
          fa: 'اردکان',
          en: 'Ardakan',
        },
      },
      {
        id: '1091',
        labels: {
          fa: 'اشکذر',
          en: 'Ashkezar',
        },
      },
      {
        id: '1092',
        labels: {
          fa: 'بافق',
          en: 'Bafq',
        },
      },
      {
        id: '1093',
        labels: {
          fa: 'بفروئیه',
          en: "Bafru'iyeh",
        },
      },
      {
        id: '1094',
        labels: {
          fa: 'بهاباد',
          en: 'Bahabad',
        },
      },
      {
        id: '1095',
        labels: {
          fa: 'تفت',
          en: 'Taft',
        },
      },
      {
        id: '1096',
        labels: {
          fa: 'حمیدیا',
          en: 'Hamidiya',
        },
      },
      {
        id: '1097',
        labels: {
          fa: 'خضرآباد',
          en: 'Khezrabad',
        },
      },
      {
        id: '1098',
        labels: {
          fa: 'دیهوک',
          en: 'Deyhuk',
        },
      },
      {
        id: '1099',
        labels: {
          fa: 'زارچ',
          en: 'Zarach',
        },
      },
      {
        id: '1100',
        labels: {
          fa: 'شاهدیه',
          en: 'Gerd Faramarz Shahediyeh',
        },
      },
      {
        id: '1101',
        labels: {
          fa: 'طبس',
          en: 'Tabas',
        },
      },
      {
        id: '1103',
        labels: {
          fa: 'عقدا',
          en: 'Aqda',
        },
      },
      {
        id: '1104',
        labels: {
          fa: 'مروست',
          en: 'Marvast',
        },
      },
      {
        id: '1105',
        labels: {
          fa: 'مهردشت',
          en: 'Mehrdasht',
        },
      },
      {
        id: '1106',
        labels: {
          fa: 'مهریز',
          en: 'Mehriz',
        },
      },
      {
        id: '1107',
        labels: {
          fa: 'میبد',
          en: 'Meybod',
        },
      },
      {
        id: '1108',
        labels: {
          fa: 'ندوشن',
          en: 'Nadushan',
        },
      },
      {
        id: '1109',
        labels: {
          fa: 'نیر',
          en: 'Nir',
        },
      },
      {
        id: '1110',
        labels: {
          fa: 'هرات',
          en: 'Harat',
        },
      },
      {
        id: '1111',
        labels: {
          fa: 'یزد',
          en: 'Yazd',
        },
      },
      {
        id: '1160',
        labels: {
          fa: 'رضوانشهر',
          en: 'Rezvanshahr',
        },
      },
    ],
  },
];

const PERSIAN_TEXT_PATTERN = /[\u0600-\u06FF]/;

const PERSIAN_TO_LATIN_MAP: Record<string, string> = {
  ا: 'a',
  آ: 'a',
  ب: 'b',
  پ: 'p',
  ت: 't',
  ث: 's',
  ج: 'j',
  چ: 'ch',
  ح: 'h',
  خ: 'kh',
  د: 'd',
  ذ: 'z',
  ر: 'r',
  ز: 'z',
  ژ: 'zh',
  س: 's',
  ش: 'sh',
  ص: 's',
  ض: 'z',
  ط: 't',
  ظ: 'z',
  ع: '',
  غ: 'gh',
  ف: 'f',
  ق: 'q',
  ک: 'k',
  گ: 'g',
  ل: 'l',
  م: 'm',
  ن: 'n',
  و: 'v',
  ه: 'h',
  ی: 'y',
  ئ: 'y',
  ء: '',
  ؤ: 'v',
  ة: 'h',
  ك: 'k',
  ي: 'y',
};

const ENDING_A_CLUSTER_CONSONANTS = new Set(['d', 'j', 'n', 'r', 'z']);
const LATIN_VOWEL_PATTERN = /^[aeiou]/i;
const LATIN_CONSONANT_PATTERN = /^(ch|gh|kh|ph|sh|th|zh|[bcdfghjklmnpqrstvwxyz])$/i;
const PERSIAN_VOWEL_LETTER_PATTERN = /[اآوی]/;

function normalizePersianLabel(value: string) {
  return value
    .trim()
    .replace(/\u200c/g, ' ')
    .replace(/[ك]/g, 'ک')
    .replace(/[يى]/g, 'ی')
    .replace(/[ؤ]/g, 'و')
    .replace(/[أإ]/g, 'ا')
    .replace(/\s+/g, ' ');
}

function isLatinVowelSound(value: string) {
  return LATIN_VOWEL_PATTERN.test(value);
}

function isLatinConsonantSound(value: string) {
  return LATIN_CONSONANT_PATTERN.test(value);
}

function transliteratePersianWord(word: string) {
  const normalizedWord = normalizePersianLabel(word);

  if (!normalizedWord) {
    return '';
  }

  const tokens: string[] = [];

  for (let index = 0; index < normalizedWord.length; index += 1) {
    const character = normalizedWord[index];
    const previousCharacter = normalizedWord[index - 1] ?? '';
    const nextCharacter = normalizedWord[index + 1] ?? '';

    if (character === 'و') {
      if (!previousCharacter) {
        tokens.push('v');
      } else if (!nextCharacter) {
        tokens.push('u');
      } else if (
        PERSIAN_VOWEL_LETTER_PATTERN.test(previousCharacter) ||
        PERSIAN_VOWEL_LETTER_PATTERN.test(nextCharacter)
      ) {
        tokens.push('v');
      } else {
        tokens.push('o');
      }

      continue;
    }

    if (character === 'ی' || character === 'ي' || character === 'ئ') {
      tokens.push(previousCharacter ? 'i' : 'y');
      continue;
    }

    if ((character === 'ه' || character === 'ة') && index === normalizedWord.length - 1) {
      tokens.push('eh');
      continue;
    }

    if (character === 'ع' && index === 0) {
      tokens.push('a');
      continue;
    }

    if (character === ' ') {
      continue;
    }

    tokens.push(PERSIAN_TO_LATIN_MAP[character] ?? character);
  }

  const explicitVowelCount = tokens.filter(isLatinVowelSound).length;
  const output: string[] = [];

  for (let index = 0; index < tokens.length; ) {
    const token = tokens[index];

    if (!isLatinConsonantSound(token)) {
      output.push(token);
      index += 1;
      continue;
    }

    let clusterEndIndex = index;
    while (clusterEndIndex < tokens.length && isLatinConsonantSound(tokens[clusterEndIndex])) {
      clusterEndIndex += 1;
    }

    const cluster = tokens.slice(index, clusterEndIndex);
    const isWordStart = index === 0;
    const isWordEnd = clusterEndIndex === tokens.length;
    const nextToken = tokens[clusterEndIndex];

    if (cluster.length === 2 && explicitVowelCount === 0) {
      output.push(cluster[0], 'a', cluster[1]);
    } else if (cluster.length === 3 && explicitVowelCount === 0) {
      if (ENDING_A_CLUSTER_CONSONANTS.has(cluster[2])) {
        output.push(cluster[0], 'a', cluster[1], 'a', cluster[2]);
      } else {
        output.push(cluster[0], 'a', cluster[1], cluster[2]);
      }
    } else if (cluster.length === 4 && explicitVowelCount === 0) {
      output.push(cluster[0], 'a', cluster[1], cluster[2], 'a', cluster[3]);
    } else if (cluster.length >= 3 && isWordStart && nextToken && isLatinVowelSound(nextToken)) {
      output.push(cluster[0], 'a', ...cluster.slice(1));
    } else if (
      cluster.length === 2 &&
      isWordEnd &&
      explicitVowelCount > 0 &&
      ENDING_A_CLUSTER_CONSONANTS.has(cluster[1])
    ) {
      output.push(cluster[0], 'a', cluster[1]);
    } else {
      output.push(...cluster);
    }

    index = clusterEndIndex;
  }

  return output.join('').replace(/aa+/g, 'a');
}

function transliteratePersianToLatin(value: string) {
  const normalized = normalizePersianLabel(value);

  return normalized
    .split(' ')
    .filter(Boolean)
    .map(transliteratePersianWord)
    .join(' ')
    .replace(/\b\w/g, (match) => match.toUpperCase())
    .replace(/\s+/g, ' ')
    .trim();
}

export function normalizeSearchText(value: string) {
  return value
    .trim()
    .replace(/\s+/g, ' ')
    .replace(/[ك]/g, 'ک')
    .replace(/[يى]/g, 'ی')
    .replace(/[ؤ]/g, 'و')
    .replace(/[أإ]/g, 'ا')
    .toLowerCase();
}

export function getLocalizedLabel(labels: LocalizedLabel, locale: Locale) {
  if (locale === 'fa') {
    return labels.fa;
  }

  if (PERSIAN_TEXT_PATTERN.test(labels.en)) {
    return transliteratePersianToLatin(labels.fa);
  }

  return labels.en;
}

export function getProvinceById(provinceId: string) {
  return iranProvinces.find((province) => province.id === provinceId);
}

export function getCityById(provinceId: string, cityId: string) {
  return getProvinceById(provinceId)?.cities.find((city) => city.id === cityId);
}

export function getLocationDisplayName(locale: Locale, provinceId: string, cityId: string) {
  const province = getProvinceById(provinceId);
  const city = getCityById(provinceId, cityId);

  if (!province || !city) {
    return null;
  }

  return `${getLocalizedLabel(province.labels, locale)} / ${getLocalizedLabel(city.labels, locale)}`;
}
