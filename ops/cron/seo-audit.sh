...
if [ "$CODE" -ne 0 ] && [ -x "$CLAUDE_BIN" ]; then
  ...
  echo "$REPORT" | "$CLAUDE_BIN" -p "You are an SEO auditor for hisense-ir.com (a Hisense-brand Next.js site, fa+en, served via next-intl). The following are automated SEO check findings. List the 3-5 most impactful ERRORS and give concrete, file/page-level fixes. Be brief." >> "$LOG" 2>&1 || true
fi
